...
- **Fast proof gate (run before every PR/push):** `PROOF_DIFF_BASE=origin/main ./script/proof.sh fast` — the single cheap local-first proof tier (target < ~10 min): whitespace/conflict markers, `script/*.py` byte-compile, the Linux-friendly harness self-tests, `check_test_coverage_manifest.sh`, core `swift test`, plus a non-blocking `check_proof_manifest.sh` report. The hosted workflow (`.github/workflows/fast-proof.yml`) is manual-only and never a PR gate. Enable the local hook with `git config core.hookspath .githooks`.
- Broad pre-beta gate: `./script/beta_readiness.sh`, plus the specific `script/check_*.sh` for the proof surface you changed.
- Pre-merge local gate: `PROOF_DIFF_BASE=origin/main ./script/proof.sh fast` — runs `swift test`, `check_test_coverage_manifest`, `check_proof_manifest`, and a whitespace check. Exits non-zero if any blocking check fails. Record its exact result and the PR head SHA; keep hosted and manual proof separate.
