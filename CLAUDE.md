...
- Pre-merge local gate: `./script/proof.sh fast` — runs `swift test`, `check_test_coverage_manifest`, `check_proof_manifest`, and a whitespace check. Exits non-zero if any blocking check fails. Record its exact result and the PR head SHA; keep hosted and manual proof separate.
