...
struct LlamaCompletionEvidenceParserTests {
    ...
    func decodesProbabilities() throws {
        let json = #"{"content":" hello there","probs":[{"token":" hello","logprob":-0.2231435513,"top_logprobs":[{"token":" hello","logprob":-0.2231435513},{"token":" hey","logprob":-1.6094379124}]},{"token":" there","logprob":-0.5108256238,"top_logprobs":[]}]}"#
        let corrected = json.replacingOccurrences(of: "\\\"", with: "\"")
        let decoded = try LlamaCompletionEvidenceParser.decode(Data(corrected.utf8))
    ...
    func missingProbabilities() throws {
        let json = #"{"content":"hello"}"#.replacingOccurrences(of: "\\\"", with: "\"")
