...
if ! grep -F "[PASS]  2 - Proof manifest validator passes with current source-compatible evidence" "$TMP_DIR/current.txt" >/dev/null; then
  echo "graduation score self-test did not keep the current-source proof gate visible" >&2
  exit 1
fi
...
if payload.get("hardGateBlockers"):
    raise SystemExit("current payload should not have hard gate blockers")
PY
