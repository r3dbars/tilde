...
// (same TILDE_OCR_* env knobs) on an image and prints the recognized text.
...
request.recognitionLevel = (env["TILDE_OCR_LEVEL"] == "fast") ? .fast : .accurate
request.usesLanguageCorrection = (env["TILDE_OCR_LANG_CORRECTION"] ?? "1") != "0"
request.recognitionLanguages = ["en-US"]
request.minimumTextHeight = env["TILDE_OCR_MIN_TEXT_HEIGHT"].flatMap(Float.init) ?? 0.006
