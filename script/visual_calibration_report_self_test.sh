...
if ! grep -F "com.apple.TextEdit / inlineAdjacent: shown=2 caretFailures=1 failureRate=33% missingCaret=1 flicker=1 learningApplied=1 latestOffset=(4.0, -2.0) trustedCorrection=applied:1 refused:1 refusedReasons=screen-changed:1" /tmp/autocomplete-visual-calibration-report-self-test.txt >/dev/null; then
