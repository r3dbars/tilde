#!/bin/bash
# The one blessed way to restart Tilde. No environment variables — the
...
DIST="$(cd "$(dirname "$0")/.." && pwd)/dist/Tilde.app"
PORT=17872
...
for _ in $(seq 1 30); do
  ...
  if curl -sf "http://127.0.0.1:$PORT/health" >/dev/null 2>&1 \
     && [ -S "$HOME/Library/Application Support/Tilde/ghost.sock" ]; then
    echo "Tilde restarted: brain socket up, engine healthy."
    exit 0
  fi
done
echo "Tilde relaunched but the engine is not healthy yet — check the menu-bar status line." >&2
exit 1
