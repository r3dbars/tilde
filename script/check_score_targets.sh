...
DEFAULT_TILDE_SCORECARD_PATH="docs/product/tilde-product-scorecard.md"
...
TILDE_SCORECARD_PATH="${AUTOCOMPLETE_LAB_TILDE_SCORECARD:-$DEFAULT_TILDE_SCORECARD_PATH}"
...
print_blocker_summary() {
  ...
  if ((TILDE_SCORECARD_ISSUES > 0)); then
    echo "- Tilde scorecard: $TILDE_SCORECARD_ISSUES issue(s). Keep the single scorecard parseable and evidence-backed before changing target docs."
  fi
...
check_tilde_scorecard() {
  local output_path
  output_path="$(mktemp)"

  if ! "$TILDE_SCORECARD_GATE_SCRIPT" --scorecard "$TILDE_SCORECARD_PATH" >"$output_path" 2>&1; then
    echo "- Tilde scorecard gate failed; target is parseable evidence-backed scorecard"
    sed -n '1,18p' "$output_path" | sed 's/^/  /'
    TILDE_SCORECARD_ISSUES=$((TILDE_SCORECARD_ISSUES + 1))
    RELEASE_ARCHITECTURE_ISSUES=$((RELEASE_ARCHITECTURE_ISSUES + 1))
    ISSUES=$((ISSUES + 1))
  fi
...
require_file "$TILDE_SCORECARD_PATH"

echo "Score target status"
echo "Tilde product scorecard: $TILDE_SCORECARD_PATH"
