...
script/geometry_trace_report.py --trace "$TRACE_FILE" --require-proof --require-layout-lanes >"$REPORT_FILE"
...
for expected in \
  ...
  "synthetic-caret=1" \
  "line=1" \
  "vertical-multi-display=1" \
  "mirrored-display=1" \
  "Stale geometry suppressions: stale-geometry-screen-layout-changed=1" \
  "healthy=1" \
...
{"type":"suggestionPresented","suggestionID":"bad","appBundleIdentifier":"com.apple.TextEdit","requestMode":"wordCompletion","latencyMilliseconds":0,"metadata":{"anchorSource":"window","anchorQuality":"diagnosticsOnly","anchorReason":"windowBoundsDiagnostics","anchorCanPresent":"true","anchorRect":"1,2,400,300","hasWindowRect":"true","geometryInvalidated":"true","geometryInvalidationReason":"window-changed"}}
JSONL
...
if ! grep -F "presented after geometry invalidation" "$BAD_REPORT_FILE" >/dev/null; then
  echo "geometry report self-test did not explain stale presented geometry" >&2
  cat "$BAD_REPORT_FILE" >&2
  exit 1
fi
