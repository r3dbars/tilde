...
{"timestamp":"2026-05-21T14:00:31.500Z","sessionID":"s","suggestionID":"p2","type":"suggestionPresented","appBundleIdentifier":"com.apple.TextEdit","fieldIdentity":"field","requestMode":"wordCompletion","displayedText":"same private text"}
...
grep -F "Immediate resurfacing: 3" "$OUTPUT_FILE" >/dev/null
...
if grep -E "secret launch text|private accepted word|raw model private|private-shot|same private text|resurfaced private text|last private text" "$OUTPUT_FILE" >/dev/null; then
...
assert report["immediate_resurfacing"] == 3
