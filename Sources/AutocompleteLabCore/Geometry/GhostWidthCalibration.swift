...
/// widths actually observed on screen. Synthetic caret placement measures
/// the typed line in a font the target app never confirmed, so the error
/// compounds with every character; one observed line width per poll pulls
/// the estimate back toward truth instead.
