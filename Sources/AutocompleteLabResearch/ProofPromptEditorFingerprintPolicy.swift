...
struct ProofPromptEditorFingerprintPolicy: Equatable, Sendable {
    func decision(
        ...
        guard PromptEditorFingerprintPolicy.dogfoodBundleIdentifiers.contains(bundleIdentifier) else {
        ...
        if Self.strongPromptTerms.contains(where: { searchable.contains($0) }) {
            ...
            guard let elementRect, let windowRect,
                  windowRect.width > 0, windowRect.height > 0 else {
        ...
        if Self.genericPromptTerms.contains(where: { searchable.contains($0) }) {
            guard let elementRect, let windowRect,
                  windowRect.width > 0, windowRect.height > 0 else {
                return PromptEditorFingerprintDecision(canSuggest: false, reason: "missing-prompt-bounds")
            }
    ...
    private func isPromptLikeGeometry(elementRect: CGRect, windowRect: CGRect) -> Bool {
        ...
        return elementRect.height >= 20
            && elementRect.height <= maxComposerHeight
            && elementRect.width >= min(260, windowRect.width * 0.35)
            && abs(windowRect.maxY - elementRect.maxY) <= windowRect.height * 0.18
    }

    private static let strongPromptTerms: Set<String> = [
        "ask codex", "ask claude", "claude message composer", "codex message composer",
        "compose a message", "describe a task or ask a question", "message composer",
        "prompt editor", "prompt input"
    ]
    private static let genericPromptTerms: Set<String> = [
        "ask", "chat", "claude", "codex", "input", "message", "prompt", "text area", "textarea"
    ]
    private static let promptCompatibleRoles: Set<String> = ["AXTextArea", "AXTextField", "AXGroup", "AXWebArea"]
    private static let promptTextInputRoles: Set<String> = ["AXTextArea", "AXTextField"]
}
