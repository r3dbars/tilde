...
| Obsidian | A- | [obsidian.png](visual-placement-screenshots/obsidian.png) | Bounded strict visual smoke is recorded for default, non-default theme, and pane lanes. Current-head long-note proof is blocked: reruns time out before `suggestion-presented` because CodeMirror reports the AX cursor before the bottom typed line. | Real CodeMirror proof shows caret-bound synthetic mirror placement, strict screenshot evidence, Tab accept, and configured full accept in disposable vault notes for the green lanes. The defined default, non-default theme, and split/side pane lanes start from the known proof note instead of whichever user note happened to be focused. | Current-head long-note proof, more vault layouts, and hidden-caret edge cases. |
...
2. Finish the remaining guarded Monaco lanes:
