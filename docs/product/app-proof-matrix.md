...
| Chrome text fields | A- | [chrome-textarea.png](visual-placement-screenshots/chrome-textarea.png), [chrome-contenteditable.png](visual-placement-screenshots/chrome-contenteditable.png) | 2 verified accepts per local fixture at 2026-05-12T09:12:22Z and 2026-05-12T09:12:38Z, plus fresh `textarea-public` public-page proof at 2026-05-12T09:15:03Z. | Local textarea/contenteditable fixtures and the bounded public EditPad textarea lane are solid. The public lane uses disposable text, guarded focus, and real Autocomplete Lab accept verification. Search, login, payment, and known hosted risky surfaces stay suppressed. | Public MediumEditor-style contenteditable proof is stale after the current rerun could not keep a stable web-backed AX editor target. |
| Browser editor fixtures | A- | [chrome-editor-like.png](visual-placement-screenshots/chrome-editor-like.png), [chrome-monaco-like.png](visual-placement-screenshots/chrome-monaco-like.png), [chrome-prosemirror-like.png](visual-placement-screenshots/chrome-prosemirror-like.png), [chrome-monaco-real.png](visual-placement-screenshots/chrome-monaco-real.png), [chrome-prosemirror-real.png](visual-placement-screenshots/chrome-prosemirror-real.png) | 2 verified accepts per fixture in the manual smoke log, including current app-binary `monaco-real` and `prosemirror-real` forced-renderer proof from 2026-05-12T09:13:45Z and 2026-05-12T09:14:02Z, normal-Chrome `prosemirror-real-default` proof at 2026-05-12T09:19:46Z, strict isolated-CDP `prosemirror-official` proof at 2026-05-12T07:47:10Z, and strict isolated-CDP `codemirror-official` proof at 2026-05-12T08:14:38Z. | Good proof for CodeMirror-like, Monaco-like, ProseMirror-like, real Monaco, and real ProseMirror under isolated forced-renderer Chrome. Normal Chrome default AX is current for real ProseMirror after the same-app focus churn fix. Normal Chrome default Monaco is no longer claimed: current reruns expose a one-character AX text area and setup verification stays unchanged. Obsidian default/theme/pane/long-note proof is current again through the dedicated proof vault. The smoke harness now uses isolated temporary Chrome profiles plus localhost DevTools setup for official CodeMirror, Monaco, and ProseMirror pages. The official ProseMirror and CodeMirror lanes are green. Monaco official display now reaches `floatingMirror` after a cramped inline frame, but verified accept remains pending because the AX-readable setup reports insertion `unchanged`, while the natural setup falls back to `tooLittleContext`. | Monaco default-AX and production editor proof still need verified insertion, not just display. |
...
2. Refresh public contenteditable proof on a stable bounded rich-text page before raising Chrome text fields back to A.
3. Finish the remaining guarded Monaco lanes:
   ...
   screenshot-backed accept evidence. The smoke harness now
   refuses concurrent runs, checks the active Chrome URL, uses isolated Chrome
   plus localhost DevTools for official-page setup, requires a focused editable
   web AX target, and still fails honestly when Monaco either exposes
   too-little-context AX state or rejects verified insertion from the
   AX-readable setup.
