...
| Obsidian | C+ | [obsidian.png](visual-placement-screenshots/obsidian.png) | 2 verified accepts exist; detached whole-editor anchors are also suppressed | The profile can work, but the proof still needs a current bounded accept run. | Needs a disposable vault note run with current bounded accept proof. |
| Apple Notes title | C | [notes-title.png](visual-placement-screenshots/notes-title.png) | Older generic Notes proof exists, but it is historical only. | The title field is its own proof target because it behaves differently from the body and checklist. The current profile is mirror-first until title/body/checklist proof exists. | Needs `AUTOCOMPLETE_LAB_SCREENSHOT_TRACE=1 script/real_app_smoke.sh notes-title --manual-gate`. |
| Apple Notes body | C | [notes-body.png](visual-placement-screenshots/notes-body.png) | Older generic Notes proof exists, but it is historical only. | The body field is its own proof target and must not borrow the title/checklist result. The current profile is mirror-first until title/body/checklist proof exists. | Needs `AUTOCOMPLETE_LAB_SCREENSHOT_TRACE=1 script/real_app_smoke.sh notes-body --manual-gate`. |
| Apple Notes checklist | C | [notes-checklist.png](visual-placement-screenshots/notes-checklist.png) | Older generic Notes proof exists, but it is historical only. | Checklist rows are their own proof target because insertion and caret behavior can differ. The current profile is mirror-first until title/body/checklist proof exists. | Needs `AUTOCOMPLETE_LAB_SCREENSHOT_TRACE=1 script/real_app_smoke.sh notes-checklist --manual-gate`. |
