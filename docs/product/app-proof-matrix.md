...
3. Finish the remaining guarded official Chrome editor lane:
   `monaco-official`. Keep browser editor fixtures non-A until that lane has
   bounded screenshot-backed trace evidence. The smoke harness now
   ...
   web AX target, and still fails honestly when Monaco's official page either
   exposes too-little-context AX state or rejects verified insertion from the
   AX-readable setup.
