...
| Codex | A- | [codex-inline.png](visual-placement-screenshots/codex-inline.png) | Historical bounded strict visual smoke at 2026-05-09T03:06:50Z has exactly 1 verified one-word accept and prompt no-submit confirmation, but it is stale against the latest app fingerprint. The 2026-05-12 current-binary rerun restored the existing draft safely but failed before proof because Tab was rejected as `focus-changed`. | Codex proves the shape of the prompt-safe lane: screenshot, one-word Tab accept, direct marked-composer insertion, insertion verification, and no submit in one trace slice. The profile stays word-only; full accept remains disabled until a separate no-submit lane exists. | Fix the current Codex focus-stability proof failure, rerun `codex --manual-gate`, then add more prompt layouts. |
| Obsidian | A- | [obsidian.png](visual-placement-screenshots/obsidian.png) | Bounded strict visual smoke refreshed at 2026-05-12T05:42:47Z with 2 verified accepts and current app fingerprint proof after opening the dedicated `ObsidianProofVault` note through an explicit URI. Non-default theme proof passed at 2026-05-12T05:44:48Z. | Real CodeMirror proof shows caret-bound synthetic mirror placement, strict screenshot evidence, Tab accept, and configured full accept in a disposable vault note. The default and theme lanes now start from the known proof note instead of whichever user note happened to be focused. Variant proof lanes still separate default, non-default theme, split/side pane, and long scrolled note layouts. | Add `obsidian-pane` and `obsidian-long-note` bounded strict screenshot-backed rows. |
...
2. Expand Obsidian coverage with the required variant lanes:
   `obsidian-pane` and `obsidian-long-note`.
