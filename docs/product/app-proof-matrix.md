...
| Obsidian long notes | word-only | Recorded long-note proof has bounded strict screenshot evidence, viewport-end repair, verified Tab insertion, and accepted-and-kept behavior. Current-head proof refresh is still required before this lane counts as complete again. |
...
| Obsidian | A- | [obsidian.png](visual-placement-screenshots/obsidian.png) | Bounded strict visual smoke is recorded for default, non-default theme, pane, and long-note lanes, but current-head refresh is stale and the font/zoom, Markdown, multiline, and run-on lanes are still pending. | Real CodeMirror proof shows caret-bound synthetic mirror placement, strict screenshot evidence, Tab accept, and configured full accept in disposable vault notes for the recorded lanes. The smoke helpers now expose the remaining variant lanes directly instead of hiding them behind the default Obsidian row. | Refresh default/theme/pane/long-note and add screenshot-backed font/zoom, Markdown bold/list, multiline, and run-on rows before raising this back to A. |
