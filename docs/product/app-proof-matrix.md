...
6. Run the guarded official Chrome editor lanes: `codemirror-official`,
   ...
   lane has bounded screenshot-backed trace evidence. The 2026-05-12
   `codemirror-official` attempt failed closed before typing because
   SteadyType was missing macOS Accessibility permission. The smoke harness now
   refuses concurrent runs, checks Accessibility before runtime readiness,
   waits up to 180s for cold official-demo MLX warmup, checks the active Chrome
   URL, fails fast when Chrome's JavaScript-from-Apple-Events setting blocks
   official-demo focus, requires a focused editable web AX target, and verifies
   Chrome setup text against the focused editor value before proceeding.
