...
| Codex layouts | proof-only | Default one-word no-submit proof refreshed at 2026-05-24T11:28:11Z; normal beta use, broader layouts, and full accept remain gated. |
...
| Codex | A- | [codex-inline.png](visual-placement-screenshots/codex-inline.png) | Current bounded strict visual smoke at 2026-05-24T11:28:11Z has exactly 1 verified one-word accept, prompt no-submit confirmation, diagnostics lines 280736-280777, trace lines 18824-18828, app proof `app-sha256:66b543983fdf0d94569e41cdb4520cdb8ddbcf69034253b918824737e8cf88d5`, and archive proof `archive-sha256:c33b4cb819b077dd5e8a75fec32155600cdca2d8fb652140b893523799496218`. | Codex has a proof-only prompt lane: screenshot, one-word Tab accept, fast-path focus/accept guard proof, direct marked-composer insertion, insertion verification, and no submit in one trace slice. The helper now prefers a disposable prompt over unrelated focused Codex goal fields and keeps private draft backup/restore for real prompt drafts. It is not beta-safe normal writing support. Full accept remains disabled until a separate no-submit lane exists. | Add more prompt layouts before using this outside proof mode. |
...
| Claude Code | A- | [claude-code-terminal.png](visual-placement-screenshots/claude-code-terminal.png) | Current bounded strict visual smoke at 2026-05-25T13:56:48Z has exactly 1 verified Terminal-hosted one-word Tab accept on the current app build, diagnostics lines 364600-364662, trace lines 24701-24709, visual `strict-complete`, prompt no-submit confirmation, app proof `app-sha256:d222bab3847ec03698e28a0f4a61a959c5301aaf085e701786c2089e650ee1ff`, and archive proof `archive-sha256:964bc381a1b98103e2a485dbeb5dd8b793a276dd759977ca83df46bb1e8188d4`. | Direct bundle support is still diagnostics-only because the installed `com.anthropic.claude-code` app is a background-only CLI helper. The proof-only terminal-host adapter maps supported terminal hosts to a virtual Claude Code profile only in explicit proof mode, blocks unmarked terminal sessions, shell prompts, command-shaped prompt lines, active agent output, stale marked scrollback, and multiline command buffers, maps the verified proof prompt back to a compose field for suggestions, carries the virtual bundle ID through traces, rechecks focus and the acceptance snapshot before insertion, and now accepts one word from phrase-mode suggestions through verified AX selected-text insertion, verified Unicode key events, or verified hardware key events for typeable one-word accepts while keeping pasteboard fallback disabled in this event-tap path. Host-labeled proof commands exist for Terminal, iTerm2, Warp, Ghostty, kitty, Alacritty, and WezTerm, and the Settings current-app panel shows the exact copied command when one of those terminal hosts is focused while keeping normal terminal suggestions blocked. | Keep full accept disabled, then record host-labeled proof for iTerm2 and Ghostty. Warp is not installed here, so it remains an honest gap. |
