...
| Claude Code | D | Pending | Pending | Direct bundle support is now diagnostics-only because the installed `com.anthropic.claude-code` app is a background-only CLI helper. | Needs a terminal-host adapter that proves one-word Tab accept without submitting shell input or an agent prompt. |
