...
| Chrome chat-like composer | C | Pending | Pending current screenshot-backed run | The local no-submit fixture exists, but it is not visually proven yet. | Needs `AUTOCOMPLETE_LAB_SCREENSHOT_TRACE=1 script/real_app_smoke.sh chrome --fixture chat-like`. |
| Codex | B- | [codex-inline.png](visual-placement-screenshots/codex-inline.png) | 2 verified accepts exist in the manual smoke log | Real dogfood screenshot exists, and insertion has passed separately. | Needs one strict visual trace slice that proves screenshot, Tab accept, full accept, and no prompt submit together. |
...
| Claude desktop | B- | Pending fresh screenshot | 2 verified accepts exist in the manual smoke log | Prior manual proof passed, but it is not current screenshot-backed proof. | Needs a current screenshot-backed prompt pass without submitting. |

## Required Next Proof

1. Run `AUTOCOMPLETE_LAB_SCREENSHOT_TRACE=1 script/real_app_smoke.sh codex --manual-gate` and keep one trace slice that proves visual placement plus Tab/full accept.
...
5. Refresh Claude desktop with screenshot tracing.
