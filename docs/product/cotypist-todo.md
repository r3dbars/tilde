...
- [x] Replace the mock runtime with a real app-owned local runtime path.
  ...
  - [x] Add a local model override so latency trials do not require code edits.
...
- [x] Build repeatable smoke checks.
  ...
  - [ ] Claude Code terminal-host adapter plus one-word no-submit accept; Terminal and iTerm2 have current host-labeled proof, Ghostty reaches display/Tab capture but fails closed on insertion, and direct `com.anthropic.claude-code` bundle proof is diagnostics-only.
