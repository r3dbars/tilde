...
The repo's existing Apple-native visual-feel score is **93/100**. This score is lower
...
| Atomic undo | 85 | Accepted insertions now arm an 8s one-step Command-Z restore for the same focused app/field; deliberate undo clears acceptance-survival tracking so normal Command-Z is not scored as accepted-then-deleted; raw accepted text stays only in ephemeral memory and diagnostics log lengths/status only. The TextEdit smoke lane now has fresh live proof at 2026-05-08T09:16:49Z with `accepted-insertion-undone`, two verified accepts, and strict visual evidence. | Prove the restore path per app beyond TextEdit and decide whether native undo grouping can replace the app-level fallback. |
