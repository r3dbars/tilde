...
| Display score | 90 | Live display score includes utility, style fit, context fit, user affinity, risk, repetition, instability, accepted-and-kept probability, behavior profile, and trace metadata. Accepted-kept learning now applies a bounded utility adjustment in addition to affinity and profile-aware low-probability suppression. Candidate count, top score, score margin, and suppression reason are logged at runtime. | Replace remaining heuristic components with learned estimates and use fresh traces to tune thresholds. |
