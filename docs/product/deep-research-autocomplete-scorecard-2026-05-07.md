...
- `SuggestionOrchestrator` now owns rich request construction, behavior-profile
  ...
  suite. AppDelegate still owns presentation, insertion, screenshot, and
...
| Architecture and tests | 2 | 96 | 1.9 | Good policy/test structure, app-proof command execution is behind a small coordinator, and request construction/session-cache/request-ticket/field-delivery/failure-visibility/fast-word suggestion orchestration is now behind `SuggestionOrchestrator`; AppDelegate still owns presentation, insertion, screenshot, and placement orchestration. |
