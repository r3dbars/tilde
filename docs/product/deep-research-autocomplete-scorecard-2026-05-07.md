...
| Candidate generation | 82 | Phrase/sentence prompts ask for 1-3 candidates; `CompletionOutputCleaner.cleanCandidates` strips list prefixes, filters unsafe/sentinel lines, dedupes, and context/profile-aware `CompletionCandidateRanker` picks only high-score/high-margin candidates while penalizing unsupported names/dates, generic filler, email commitments, coding block/API drift, prompt-app submit actions, and form/search fills. | Prove real model outputs produce useful candidate sets and tune score/margin thresholds from traces. |
