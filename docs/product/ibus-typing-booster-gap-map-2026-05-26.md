...
| No explicit summon-on-demand product mode | IBus `tabenable` is valuable, but Tilde cannot use Tab for summon without breaking Tab accept. | Future: `KeyboardAction`, `AppSettings`, `SuggestionTriggerPolicy`, Settings UI |
| Learned suggestion removal is not first-class enough | IBus lets users remove learned candidates. Tilde has learning stores, but not a crisp "never suggest this again" UI. | Future: `SuggestionRepetitionSuppressor`, `PersonalCaptureEpisodeStore`, Diagnostics/Settings |
