...
current Tilde app bundle. The newest red evidence keeps the score flat:
...
whether Tab capture itself poisons the prompt before Tilde insertion starts.
...
That lane now has proof-only accept-driver evidence. `20260528T230747Z-ghostty`
proved the old key-probe fallback waited too long: the run reached a prompt-row
suggestion, then exhausted the key-capture probe ladder before posting the
accept command, and Tilde refused it with `hasVisibleSuggestion=false`.
The patched follow-up, `20260528T231520Z-ghostty`, bypassed that ladder when
`AUTOCOMPLETE_LAB_CLAUDE_CODE_GHOSTTY_PROOF_ONLY_ACCEPT_DRIVER=1`, posted
`--proof-only-accept-next-word` while the prompt-row suggestion was visible, and
logged `proof-only-accept-command-result ... handled=true` at diagnostics line
`1080242`. It still failed red after scheduling deferred insertion: the opt-in
post-Tab/pre-insert native comparator could not restore the prompt, and the
first app-owned insertion rung timed out at line `1080284`. So Tab delivery is
no longer the active blocker for this proof-only lane; verified Ghostty
insertion remains the blocker.
The current-code wrapper check `20260528T232204Z-ghostty` then showed the
proof-only lane must stay lean: with the slower pre-accept System Events
comparator enabled by default, the prompt-row suggestion hid before the
proof-only command arrived, and Tilde refused it with
...
detached-runner key source that reaches the Tilde event tap at all.
...
synthetic key source that might reach Tilde's event tap.
...
Tilde`, but the signal snapshot named a separate Codex-owned stop command
...
permission-safe key-capture sentinel missed Tilde's event tap: session
...
reaches Tilde's event tap for Ghostty, not prompt readiness, placement,
or external TERM interference.
The follow-up source slice makes Tilde's keyboard tap location
...
without turning Ghostty green. Tilde now launches Ghostty `input text`
...
Ghostty green. Tilde now tries Ghostty's native `input text` through an
...
support. Tilde now paces app-owned Command-V, hardware, and Unicode-to-pid
...
forwards that opt-in env into both the worker and relaunched Tilde app.
...
`./script/claude_code_ghostty_detached_proof_self_test.sh`,
`./script/check_tilde_scorecard.py --live`, and `git diff --check` passed
...
past that handoff failure: the child printed the smoke plan, built Tilde,
...
AppleScript identity without broadening support. `TildeTextEventHelper`
...
Tilde now preserves the AppleScript `perform action` boolean for focused
...
so Tilde tries a smoke-equivalent frontmost bundle System Events raw
...
Tilde posted `ghosttyPrePromptFocusBundleSystemEventsRawKeystroke` at
...
Tilde's event tap through either session or HID CGEvents, so the harness
...
next Ghostty blocker is observable key delivery into Tilde's event tap
...
red bar before it could retest the key source: Tilde's terminal prompt
...
enough because the screen-copy check stayed quiet while Tilde continued to
log the full typed prompt length. The harness now has a second typed-readiness
fallback that accepts Tilde's privacy-safe terminal prompt-anchor
...
| Naming and public docs | Tilde-facing copy, old lab/tester wording, support claims. | Public docs should say Tilde and avoid broad support promises. |
...
```bash
./script/check_tilde_scorecard.py --live
