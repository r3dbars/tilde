...
| Visual placement and caret alignment | 18 | 65 | 100 | Stale async suggestions refresh focused geometry before display, unusable panels suppress before key capture, Chrome chat-like now has proof, and Diagnostics exposes placement confidence without suggestion text. Notes, Obsidian, Claude Code, and Claude desktop are still the blocker. |
...
| User control | 6 | 94 | 100 | Settings and the menu now expose pause, current-field silence, app blocking, support status, per-app render mode, force-mirror override, privacy diagnostics, temporary raw/screenshot capture, local log deletion, shortcut state, and why the last suggestion was hidden. Shortcut editing and proof collection remain open. |
