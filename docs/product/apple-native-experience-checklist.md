...
| Privacy and permissions trust | 9 | 94 | 100 | Local-first and redaction are strong, recent-word memory no longer crosses app boundaries, raw/screenshot debug capture now expires when enabled from the app UI, Settings has a one-click Privacy Status panel, local log deletion clears compatibility-learning artifacts, raw text capture now says it can include what you type, and exported reports include a privacy checklist. Broader first-run permission proof remains open. |
...
| Failure restraint | 8 | 87 | 100 | Slow polling can hide suggestions, repeated slow app-specific AX reads cool down, stale geometry suppresses display, unknown AX field kinds suppress unless a profile explicitly opts in, event-tap disablement fails closed, prompt full accept requires proof, and unsupported apps now explain their stance. Low-confidence inline mode still needs stricter real-app gating. |
