...
| Codex | dogfood target | synthetic inline caret, no detached fallback | AX value replacement, key fallback | degraded one-word only until no-submit undo proof exists | prior manual pass is stale for this gate; current one-word no-submit proof pending |
