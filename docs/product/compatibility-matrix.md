...
| Claude Code | proof-only terminal-host target | synthetic inline caret, no detached fallback | verified AX/Unicode first, hardware key-event fallback for typeable one-word accepts | unavailable for direct bundle; terminal-host proof needed | recorded current strict visual one-word no-submit Terminal proof; pasteboard fallback stays disabled in the event-tap path; direct `com.anthropic.claude-code` bundle remains diagnostics-only because real typing happens in a terminal host |
