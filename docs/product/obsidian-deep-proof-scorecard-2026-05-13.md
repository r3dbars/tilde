...
| Deep repeat reliability | 45 | The delayed theme lane is green, but the latest subset sweep was interrupted by a stale `25ed` Tilde app instance and produced no trustworthy rows. The requested 150+ Obsidian attempt sample is not complete. |
...
- Made Obsidian marker matching whitespace-tolerant because zoomed Obsidian can expose `Tilde Obsidian writing proof` as `Tilde \nObsidian proof` through AX.
...
- The proof launcher now collapses duplicate current `Tilde` processes before typing, so a stale same-path app instance cannot contribute shadow trace events.
...
- Exclusive proof runs now terminate stale Tilde app bundles from other worktrees while waiting for the current app and while the interference guard is active.
...
- Earlier foreground Computer Use Tab proof was red: `placement-proof.md` contained `Tilde Obsidian writing proof\n\n[TAB]Smoke proof fee` after Tab.
- Fresh foreground Computer Use Tab proof is green: `placement-proof.md` contained `Tilde Obsidian writing proof\nSmoke proof feed` after Tab, with no `[TAB]`.
...
- Latest long-note foreground proof is green: Tilde presented an Obsidian suggestion at the visible tail after `obsidian-codemirror-line-start-tail` repair, accepted two suffixes, and recorded verified insertion.
...
- The first font/zoom run exposed a placement failure: the disposable proof note became `Tilde \nSmoke proof feelsObsidian proof`, so accepted/typed text was not at the intended visual end.
...
- A separate `tilde-score-loop` heartbeat in the old `25ed` worktree repeatedly flipped proof mode to TextEdit and killed/invalidated Obsidian proof attempts. That heartbeat was paused during this goal.
...
- Fresh file evidence: `/Users/redbars/Documents/claudebrain-lab/Tilde Obsidian Proof Codex.md` kept the long-note body and ended with `Long note filler line 90 ... Smoke proof feels`.
- Fresh bad evidence: `/Users/redbars/Documents/claudebrain-lab/Tilde Obsidian writing proof.md` ended up as `Smoke proof feelsTilde Obsidian writing proof`, which is not acceptable insertion behavior.
...
- The latest screenshot trace artifacts include `/Users/redbars/Library/Logs/Tilde/screenshots/DF696F28-2CD6-439E-BE0E-C69CB3615E3A.png`, `/Users/redbars/Library/Logs/Tilde/screenshots/A8FF3AF7-2BE8-45AF-BB4B-1ED412031C6C.png`, and `/Users/redbars/Library/Logs/Tilde/screenshots/4A55A7D2-4457-4C0A-94C2-6E2FEC2581D8.png`.
...
- The latest screenshot trace artifacts for that proof are `/Users/redbars/Library/Logs/Tilde/screenshots/4855D777-547F-4D04-86E8-B65B272B99DC.png` and `/Users/redbars/Library/Logs/Tilde/screenshots/C67D31F4-6965-4EE3-955C-13AC2539E97B.png`.
...
- The reset harness now closes duplicate Obsidian proof panes before non-pane lanes, because Computer Use showed two `placement-proof` panes and text landing inside `Tilde Obsidian writing proof` instead of after it.
...
- The 11:36 UTC default/theme/pane subset sweep is red as runner evidence: it reached `obsidian-theme`, then a stale `25ed` Tilde app instance entered the proof process group and the lane fell into focus/quiet-mode behavior instead of producing a clean row.
...
- The lingering old `25ed` `manual_proof_refresh.sh --run --target obsidian-theme` process group was terminated, and this checkout rebuilt `dist/Tilde.app` successfully afterward.
