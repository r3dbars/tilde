...
- It reached the `obsidian-theme` lane and showed the old failure mode again: a stale `25ed` Tilde app instance (`/Users/redbars/.codex/worktrees/25ed/.../dist/Tilde.app`) entered the proof process group and the lane stopped producing a clean result.
...
- The runner was patched after this miss so exclusive proof runs terminate stale Tilde app bundles from other worktrees while waiting for the current app and while the interference guard is active.
...
- After cleanup, the current checkout rebuilt `dist/Tilde.app` successfully with `script/build_and_run.sh bundle-only`.
