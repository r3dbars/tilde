...
| TextEdit support | 10/10 | Fresh bounded screenshot-backed runs on 2026-05-09 cover default, light, dark, wrapped-line, narrow-window, and scrolled-document TextEdit lanes with two verified accepts each and current proof fingerprints. The smoke harness respects the active full-accept shortcut, opens a unique disposable file, targets that exact AX window/title while other TextEdit windows exist, seeds only that disposable text area, and dismisses TextEdit's native inline completion before waiting for Autocomplete Lab. |
...
| Obsidian support | 9.25/10 | Default Obsidian has strict visual proof with two verified accepts on `qwen3-0.6b`, but that row is stale after later source changes and needs a current-head refresh. The fix proved that the old `afterChars=4` failure was real typed text, then moved Obsidian to exact AX value replacement plus same-pid descendant verification so CodeMirror's stale AX focus/selection does not turn a good insertion into a false failure. The `obsidian-theme`, `obsidian-pane`, and `obsidian-long-note` lanes remain honest proof gaps. |
...
| Real-app smoke | 9.9/10 | TextEdit, Apple Notes title/body/checklist, core Chrome fixtures, public Chrome textarea/contenteditable, official Chrome CodeMirror, real Chrome Monaco/ProseMirror under isolated renderer-accessibility mode and normal Chrome default AX, Chrome chat-like no-submit, Codex one-word no-submit, and default/theme/pane Obsidian proof are green on current proof rows. Obsidian long-note, Claude Code host variants, and Claude desktop layout lanes remain honest current-head proof gaps. |
...
| Chrome editor-like | 9.2/10 | [chrome-editor-like.png](visual-placement-screenshots/chrome-editor-like.png), [chrome-codemirror-official.png](visual-placement-screenshots/chrome-codemirror-official.png) | CodeMirror-style fixture aligns well after the caret, and the official CodeMirror demo now has bounded strict screenshot evidence with two verified accepts. | Pending: official Monaco and ProseMirror remain blocked by Chrome AX/Apple Events proof setup. |
