...
- `AUTOCOMPLETE_LAB_SCREENSHOT_TRACE=1 ./script/real_app_smoke.sh chrome --fixture monaco-real --chrome-accessibility default`: failed on 2026-05-08 before suggestion presentation; diagnostics lines 174797-174803 show normal Chrome enabled but blocked by no editable text field.
- `AUTOCOMPLETE_LAB_SCREENSHOT_TRACE=1 ./script/real_app_smoke.sh chrome --fixture prosemirror-real --chrome-accessibility default --skip-build`: failed on 2026-05-08 before suggestion presentation with the same default-Chrome no-editable-field blocker.
...
3. Test real production Monaco, ProseMirror, and CodeMirror apps, not just local
   fixtures.
4. Add in-app automated post-enable proof so first-run setup can run and verify
   TextEdit insertion and placement before the user tries riskier apps.
