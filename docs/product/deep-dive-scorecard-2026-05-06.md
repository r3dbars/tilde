...
Chrome production editor variants, and more Obsidian/editor shapes.
...
| Visual caret alignment | 9.75/10 | TextEdit, core Chrome fixtures, Chrome chat-like, real Monaco/ProseMirror under default Chrome AX, Obsidian, Apple Notes title/body/checklist, a disposable Codex prompt, Claude Code, and Claude desktop now have screenshot-backed proof. Stale line rects are dropped, vertical clipping is enforced, too-narrow inline space suppresses display instead of showing a sliver, async suggestions refresh current geometry before display, low-confidence mirror fallback is now suppressed for untrusted yellow profiles instead of showing detached element/window ghosts, and default-Chrome real-editor proof now uses proof-gated inline synthetic-caret placement instead of element-anchored mirror fallback. Claude Code's terminal-host proof now has strict visual evidence and verified one-word no-submit insertion. Codex still needs same-slice accept/no-submit proof, and more prompt/editor layouts remain open. |
...
| Claude Code support | 9.2/10 | Direct bundle support is still diagnostics-only because `com.anthropic.claude-code` is a background-only CLI helper on this machine. A proof-only terminal-host adapter now maps supported terminal hosts to a virtual Claude Code profile only when proof mode is active, validates the disposable marker/current input line/shell-prompt guard, carries the virtual bundle ID through suggestion traces, rechecks focus before insertion, inserts through the terminal host's own Paste menu via AX, and keeps full accept disabled. Bounded strict visual smoke at 2026-05-08T17:04:17Z proved one verified Tab one-word accept in Terminal-hosted Claude Code without shell or agent submit. More terminal hosts and full-accept no-submit proof remain open. |
...
| Real-app smoke | 9.8/10 | TextEdit, core Chrome fixtures, real Chrome Monaco/ProseMirror under isolated renderer-accessibility mode, default-Chrome real Monaco/ProseMirror AX plus proof-gated inline synthetic-caret lanes, Chrome chat-like no-submit, Obsidian, Apple Notes title/body/checklist, Claude Code, and Claude desktop are green on the current build. The latest strict visual smokes passed after the accept-all shortcut/race fix, typed-through word-completion fix, retention-classifier fix, synthetic-caret trust tightening, prompt proof-mode synthetic-caret gating, Claude desktop centered-composer tuning, Notes text-context repair, default-Chrome real-editor caret-quality proof runs, and Claude Code terminal-host AX menu paste proof. Codex remains an honest prompt-app proof gap. |
...
| Codex | 8.5/10 | [codex-inline.png](visual-placement-screenshots/codex-inline.png) | Disposable prompt screenshot shows the ghost visible on the same line after the caret on a negative-origin side display, but the profile stays diagnostics-only until no-submit proof is captured in the same slice. | Pending: needs a recorder-grade visual pass with one-word accept and no-submit proof in the same trace slice. |
...
| Claude Code | 9.2/10 | [claude-code-terminal.png](visual-placement-screenshots/claude-code-terminal.png) | Bounded strict visual smoke at 2026-05-08T17:04:17Z proves inline terminal-host placement plus a one-word Tab accept in the real Claude Code prompt without shell or agent submit. The adapter stays proof-only, marker-gated, shell-prompt guarded, and full-accept disabled. | Pending: more terminal hosts and separate full-accept no-submit proof before broad prompt support. |
