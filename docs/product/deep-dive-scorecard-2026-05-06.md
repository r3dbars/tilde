...
| Output relevance | 9.1/10 | Prompt labels, instruction echoes, assistant filler, unsafe prompt actions, punctuation suffixes, parroting, recommendation/rewrite/next-action starters, visible typed-word duplicates, phrase restarts, and more assistant-y prefixes are suppressed before display. Dogfood prompts now avoid loose substring triggers, but default redacted tracing means deeper output-quality audits require explicit raw-content dogfood runs. |
...
| Non-annoyance | 9/10 | Esc, typed-over tracking, repetition suppression, a 3+ typed-letter floor for word completion, visible placement-uncertainty quiet mode, pause control, insertion recovery, app-specific AX cooldowns, single slow-AX-read throttle, Settings "why hidden" copy, and untrusted low-confidence mirror suppression help, but visual misses still make the app feel annoying when real-app placement proof is missing. |
