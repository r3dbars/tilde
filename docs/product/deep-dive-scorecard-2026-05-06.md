...
terminal-hosted Claude Code, Chrome production editor variants, and more Obsidian/editor shapes.
Codex has a real screenshot now, but not screenshot plus verified one-word
accept/no-submit proof in one strict proof slice. The evidence-backed score
should stay lower until those rows are closed.
...
| Visual caret alignment | 9.7/10 | TextEdit, core Chrome fixtures, Chrome chat-like, real Monaco/ProseMirror under default Chrome AX, Obsidian, Apple Notes title/body/checklist, a disposable Codex prompt, and Claude desktop now have screenshot-backed proof. Stale line rects are dropped, vertical clipping is enforced, too-narrow inline space suppresses display instead of showing a sliver, async suggestions refresh current geometry before display, low-confidence mirror fallback is now suppressed for untrusted yellow profiles instead of showing detached element/window ghosts, and default-Chrome real-editor proof now uses proof-gated inline synthetic-caret placement instead of element-anchored mirror fallback. Claude desktop's current one-line composer proof is same-baseline with screenshot detector offset near zero. Codex still needs same-slice accept/no-submit proof, Claude Code now has a terminal-host lane but still needs screenshot proof, and more prompt/editor layouts remain open. |
...
| Screenshot tracing | 9.5/10 | Screen Recording is preflighted, capture runs off the hot path, screenshots include editor bounds plus ghost text, traces/logs include capture rect plus rendered panel rect, capture has a backlog guard plus timeout, and fresh default-Chrome real-editor traces prove inline synthetic-caret placement with screenshot offset detection. |
...
| Chrome Monaco-like support | 9.65/10 | Fresh local Monaco-like proof verifies insertion, `monaco-real` has isolated renderer-accessibility proof, and `monaco-real-default` now has strict default-Chrome proof at 2026-05-08T12:45:53Z with two verified accepts, current proof fingerprints, `effectiveRenderMode=inlineAdjacent`, `placementAnchorSource=synthetic-caret`, and a concrete text-line rect. Production editor variants remain open. |
| Chrome ProseMirror-like support | 9.7/10 | Fresh local ProseMirror-like proof verifies insertion, `prosemirror-real` has isolated renderer-accessibility proof, and `prosemirror-real-default` now has strict default-Chrome proof at 2026-05-08T12:46:09Z with two verified accepts, current proof fingerprints, `effectiveRenderMode=inlineAdjacent`, `placementAnchorSource=synthetic-caret`, and a concrete text-line rect. Production editor variants remain open. |
| Chrome chat-like no-submit support | 9.1/10 | A local no-submit fixture now has screenshot-backed proof with Tab/full accept verified and submit count still zero. Browser-hosted Slack and Discord are blocked by surface policy until real no-submit proof exists, so the local fixture no longer implies broad chat enablement. It is still a local fixture, not proof for real chat apps. |
...
| Claude Code support | 5.6/10 | Direct bundle support is still diagnostics-only because `com.anthropic.claude-code` is a background-only CLI helper on this machine. A proof-only terminal-host adapter now maps supported terminal hosts to a virtual Claude Code profile only when proof mode is active, validates the disposable marker/current input line/shell-prompt guard, carries the virtual bundle ID through suggestion traces, and keeps full accept disabled. It still needs live screenshot-backed one-word no-submit proof before support can count higher. |
...
| Output relevance | 9.2/10 | Prompt labels, instruction echoes, assistant filler, unsafe prompt actions, punctuation suffixes, parroting, recommendation/rewrite/next-action starters, visible typed-word duplicates, phrase restarts, and more assistant-y prefixes are suppressed before display. Accepted-kept display thresholds are now stricter for prompt/chat and sentence-like prose profiles after enough local samples, and quiet/normal/eager control tunes cadence plus display thresholds. Dogfood prompts avoid loose substring triggers, but default redacted tracing means deeper output-quality audits require explicit raw-content dogfood runs. |
| Word completion quality | 9/10 | Word completion and partial acceptance are useful, bounded, app-scoped, activation and fast ranking both require 3+ typed letters, fast completions obey repeated-miss suppression, and unrelated whole-word completions are rejected. It still needs more real-app miss-rate proof before scoring higher. |
| Non-annoyance | 9.15/10 | Esc, typed-over tracking, repetition suppression, a 3+ typed-letter floor for word completion, quiet/normal/eager control, profile-aware accepted-kept display restraint, visible placement-uncertainty quiet mode, pause control, insertion recovery, app-specific AX cooldowns, single slow-AX-read throttle, explicit copy-only fallback status for unsafe inline cases, Settings "why hidden" copy, and untrusted low-confidence mirror suppression help, but visual misses still make the app feel annoying when real-app placement proof is missing. |
...
| Automated tests | 10/10 | `swift test` passes 701 tests, including app-target settings state tests, app-proof command runner and coordinator tests, Settings proof action dispatch, command fallback policy tests, suggestion aggressiveness tests, suggestion orchestrator request/ticket delegation, orchestrator rich request construction, orchestrator session-cache metadata, orchestrator app-model candidate metadata, orchestrator display-score and prefix-cooldown decisions, orchestrator failure visibility gating, orchestrator field-delivery race gating, orchestrator engine replacement after runtime reload, current-field silence copy, per-app mirror override copy, direct shortcut editing copy, onboarding copy tests, diagnostics typing-health tests, prompt-context diagnostics tests, placement diagnostics tests, typed-through suggestion progress, word-completion suffix survival, 3-letter word-completion floor, profile-aware fresh-paragraph trigger suppression, accepted-kept utility adjustment, profile-aware accepted-kept display thresholds, duplicate/restart quality guards, static prompt cache metadata, trace-safe document/window title shape, prefix-family HMAC trace metadata, Claude Code terminal-host proof profile and scrollback input-line extraction, browser hosted-surface blocks, Chrome automatic proof-plan coverage, acceptance visible-text proof, runtime session-cache policy and trace metadata, Notes text-context repair, scoped recent-word memory, privacy expiry, support status, serial AX reader, focused AX-health cooldown, missing-context AX cooldown, single slow-AX-read throttle, active-typing focused-poll cadence, focused-poll backoff, dogfood false-positive coverage, neutral word-completion vocabulary, directive-output suppression, fast-word candidate metadata, screenshot trace capture policy, pixel offset detector policy, synthetic caret centering, proof-gated Chrome synthetic-caret trust, profile-aware placement trust policy, replay proof, smoke-slice replay proof, and trace visual evidence. Script self-tests now cover strict score targets, the 10-pass score loop path, manual smoke status, visual proof, replay slicing, replay profiles, typing performance, and the 10-minute endurance soak command. |
| Real-app smoke | 9.7/10 | TextEdit, core Chrome fixtures, real Chrome Monaco/ProseMirror under isolated renderer-accessibility mode, default-Chrome real Monaco/ProseMirror AX plus proof-gated inline synthetic-caret lanes, Chrome chat-like no-submit, Obsidian, Apple Notes title/body/checklist, and Claude desktop are green on the current build. The latest strict visual smokes passed after the accept-all shortcut/race fix, typed-through word-completion fix, retention-classifier fix, synthetic-caret trust tightening, prompt proof-mode synthetic-caret gating, Claude desktop centered-composer tuning, Notes text-context repair, and default-Chrome real-editor caret-quality proof runs. Codex remains an honest prompt-app proof gap; Claude Code now has a manual-gated terminal-host proof lane but still needs live screenshot/no-submit evidence. |
...
| Architecture | 9.6/10 | Core policy, geometry, scoped word memory, trace analysis, privacy expiry, support status, serial AX focused-text reads, AX-health cooldowns, app-proof command coordination, and suggestion request/ticket/request-construction/candidate-metadata/display-score/prefix-cooldown/field-delivery/failure-visibility orchestration are tested and wired. AppDelegate still owns presentation, insertion, screenshot, and placement orchestration. |
...
| Chrome textarea | 9/10 | [chrome-textarea.png](visual-placement-screenshots/chrome-textarea.png) | Inline ghost is readable and follows the typed text; untrusted low-confidence mirror fallback is now suppressed instead of showing detached yellow-profile placement. | Pending: real website proof beyond the local fixture. |
...
| Chrome Monaco-like | 9.65/10 | [chrome-monaco-like.png](visual-placement-screenshots/chrome-monaco-like.png), [chrome-monaco-real.png](visual-placement-screenshots/chrome-monaco-real.png) | Local Monaco-like proof is readable near the caret, and real Monaco now has strict screenshot evidence in both isolated forced-renderer Chrome and default Chrome with Tab, Option-Tab, proof-gated inline synthetic-caret placement, and a concrete text-line rect. | Pending: production editor variants. |
| Chrome ProseMirror-like | 9.7/10 | [chrome-prosemirror-like.png](visual-placement-screenshots/chrome-prosemirror-like.png), [chrome-prosemirror-real.png](visual-placement-screenshots/chrome-prosemirror-real.png) | Local ProseMirror-like proof is inline, and real ProseMirror now has strict screenshot evidence in both isolated forced-renderer Chrome and default Chrome with Tab, Option-Tab, proof-gated inline synthetic-caret placement, and a concrete text-line rect. | Pending: production editor variants. |
...
| Claude Code | 5.6/10 | Pending terminal-host proof | Direct bundle is diagnostics-only. The proof-only terminal-host adapter now has unit coverage for proof-mode gating, marker checks, current input-line extraction from terminal scrollback, and shell-prompt rejection. | Needs live screenshot-backed one-word no-submit proof through `script/real_app_smoke.sh claude-code --manual-gate`. |
...
- Suggestion orchestrator refactor pass: `swift test --filter
  SuggestionOrchestratorTests` passed 14 focused app tests after moving active
  ...
  construction, prefix-cooldown display pressure, field-delivery race gating,
  failure visibility gating, fast word-selection delegation, engine delegation,
  and runtime-reload engine replacement behind `SuggestionOrchestrator`.
...
- `./script/manual_smoke_status.sh --strict`: now shows Chrome chat-like no-submit, Obsidian, Notes title/body/checklist, and Claude desktop as passed, and fails honestly on Codex prompt proof, Claude Code terminal-host proof, and remaining score targets. The current blockers remain Codex same-slice insertion proof and live Claude Code terminal-host screenshot/no-submit proof.
...
- `./script/check_visual_placement_evidence.sh`: passed with 15 verified visual-placement screenshots and reports one pending screenshot audit plus one stale same-slice Codex row.
- `./script/check_visual_placement_evidence.sh --require-all`: fails honestly on the remaining Claude Code screenshot audit and Codex same-slice proof gap.
...
- `AUTOCOMPLETE_LAB_SCREENSHOT_TRACE=1 ./script/real_app_smoke.sh chrome --fixture monaco-real --chrome-accessibility default`: passed at 2026-05-08T12:45:53Z with default Chrome Accessibility exposure, strict screenshot evidence, Tab accept, Option-Tab full accept, two verified insertions, current proof fingerprints, `effectiveRenderMode=inlineAdjacent`, `placementAnchorSource=synthetic-caret`, `placementHealthReason=healthy`, and a concrete text-line rect.
- `AUTOCOMPLETE_LAB_SCREENSHOT_TRACE=1 ./script/real_app_smoke.sh chrome --fixture prosemirror-real --chrome-accessibility default --skip-build`: passed at 2026-05-08T12:46:09Z with default Chrome Accessibility exposure, strict screenshot evidence, Tab accept, Option-Tab full accept, two verified insertions, current proof fingerprints, `effectiveRenderMode=inlineAdjacent`, `placementAnchorSource=synthetic-caret`, `placementHealthReason=healthy`, and a concrete text-line rect.
...
1. Run safe terminal-host proof for Claude Code through the new proof-only
   adapter before recording it as supported. Codex has visual proof now, but
   still needs one-word accept/no-submit proof in the same strict visual trace
   slice. Claude desktop now has same-baseline no-submit proof, but needs more
   prompt-layout coverage.
