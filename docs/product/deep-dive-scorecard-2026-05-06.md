...
for the remaining prompt-app and Obsidian variants, especially additional Codex
and Claude desktop layouts plus current-head Obsidian long-note proof. The latest
app-code source change also makes most older recorded proof rows stale under the
strict current-head gate. The evidence-backed score should stay lower until
those rows are refreshed or closed.
...
| Acceptance reliability | 9.6/10 | TextEdit, core Chrome fixtures, Chrome chat-like, public Chrome textarea/contenteditable, isolated real Monaco/ProseMirror rows, normal-Chrome real ProseMirror default-AX rows, Apple Notes title/body/checklist, Obsidian default/theme/pane, Codex one-word no-submit, Claude Code default Terminal-host, and Claude desktop default composer verify accept paths on recorded proof rows. The latest strict current-head gate now requires refresh for most lanes after the Claude desktop insertion source change. Prompt-app full accept is intentionally disabled until separate full-accept no-submit proof exists, and a profile-aware safety gate blocks no-submit profiles from full accept, multiword accepted text, non-visible accepted text, and newline/tab/control accepted text before insertion. Selected text is blocked before suggestions/acceptance, AX insertion is faster, full accept has trace-safe exact-visible-text proof, Tab accept is traced as a visible-prefix accept, after-cursor drift fails verification, and missing/changed post-write verification targets are recorded as insertion failures instead of disappearing. Normal-Chrome Monaco, Obsidian long-note current-head proof, and Claude desktop layout lanes remain honest proof gaps. |
| Visual caret alignment | 9.55/10 | TextEdit, core Chrome fixtures, Chrome chat-like, public Chrome textarea/contenteditable, real Monaco/ProseMirror under isolated forced-renderer Chrome, normal Chrome default-AX ProseMirror, Apple Notes title/body/checklist, Obsidian default/theme/pane, Codex, Claude Code, and Claude desktop default composer have screenshot-backed recorded proof. Most rows still need current-head strict refresh after the latest app-code source change. Stale line rects are dropped, vertical clipping is enforced, too-narrow inline space suppresses display instead of showing a sliver, async suggestions refresh current geometry before display, and low-confidence mirror fallback is now suppressed for untrusted yellow profiles instead of showing detached element/window ghosts. Normal-Chrome Monaco, Obsidian long-note, and more prompt-app layouts still need strict rows. |
...
| Chrome contenteditable support | 9.6/10 | Fresh local full-frame screenshot and two verified accepts are current, and the public MediumEditor-style lane now has strict screenshot-backed proof after the verifier allowed the same AX editor target to collapse to a single editing line. More public rich-text domains would make this stronger. |
...
| Obsidian support | 9.35/10 | Default, non-default theme, and split/side pane lanes have recorded strict visual proof with two verified accepts each in the disposable proof vault. Current-head long scrolled-note proof is blocked because the live harness sees CodeMirror report the AX cursor before the bottom typed line and times out before `suggestion-presented`. The harness seeds the proof note before opening Obsidian, keeps user vaults out of scope, resets pane variants between accepts when needed, and uses exact AX value replacement plus same-pid descendant verification so CodeMirror focus churn does not turn a good insertion into a false failure. Broader vault layouts and hidden-caret edge cases remain useful follow-up proof. |
...
| Claude Code support | 9.35/10 | Direct bundle support is still diagnostics-only because `com.anthropic.claude-code` is a background-only CLI helper on this machine. A proof-only terminal-host adapter maps supported terminal hosts to a virtual Claude Code profile only when proof mode is active, validates the disposable marker/current input line/shell-prompt guard, rejects command-shaped prompt lines, active agent output, stale marked scrollback, and multiline command buffers, carries the virtual bundle ID through suggestion traces, rechecks focus and the acceptance snapshot before insertion, inserts through the terminal host's own Paste menu via AX, and keeps full accept disabled. Recorded strict visual rows at 2026-05-12T10:58:42Z and 2026-05-12T11:04:20Z prove one verified Tab one-word accept in Terminal-hosted Claude Code without shell or agent submit; they now need current-head refresh. iTerm2 and Ghostty are installed but still need host-labeled proof rows; Warp is not installed here. |
...
| Real-app smoke | 9.2/10 | TextEdit, Apple Notes title/body/checklist, core Chrome fixtures, public Chrome textarea/contenteditable, real Chrome Monaco/ProseMirror under isolated renderer-accessibility mode, normal Chrome default-AX ProseMirror, Chrome chat-like no-submit, Obsidian default/theme/pane, Codex one-word no-submit, Claude Code default Terminal-host no-submit, and Claude desktop default composer all have bounded recorded proof rows. `./script/manual_smoke_status.sh --strict` now requires 29 current-head proof refreshes after the latest app-code source change. Normal-Chrome Monaco, Obsidian long note, and Claude desktop layout variants remain honest current-head proof gaps. |
...
| Obsidian | 9.35/10 | [obsidian.png](visual-placement-screenshots/obsidian.png) | Default, non-default theme, and pane Obsidian lanes have recorded strict visual rows with two verified accepts each. Direct AX value replacement plus descendant verification handles CodeMirror stale cursor/focus churn without changing models. | Pending: current-head long-note proof, then broader vault layouts and hidden-caret edge cases. |
...
| Claude Code | 9.5/10 | [claude-code-terminal.png](visual-placement-screenshots/claude-code-terminal.png) | Current no-submit smoke at 2026-05-12T06:41:56Z on `ca41cdc33e5e` proves a one-word Tab accept in the real Terminal-hosted Claude Code prompt without shell or agent submit; the row is insertion-only because the fresh signed test app hit a macOS screen/audio prompt that was not granted. Historical strict visual smoke at 2026-05-08T17:04:17Z proves inline Terminal-host placement. The adapter stays proof-only, marker-gated, shell-command guarded, active-output guarded, stale-focus rechecked, host-labelable, full-accept disabled, and now handles current marked prompt lines even when Terminal scrollback contains prior shell commands. | Pending: strict screenshot refresh after user-granted Screen Recording, host-labeled iTerm2 and Ghostty proof rows on this Mac; Warp is not installed here; separate full-accept no-submit proof stays blocked. |
