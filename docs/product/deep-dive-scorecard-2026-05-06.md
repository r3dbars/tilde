...
| Onboarding | 8.8/10 | Settings explains runtime readiness, current app state, local privacy controls, and Accessibility setup in one short paragraph. Screen Recording copy appears only when screenshot capture is enabled, first success points to TextEdit, and missing/invalid app-owned MLX assets can now install or repair in-app with progress before runtime warm. Cancel/retry/offline polish remains open. |
...
3. Test real production Monaco, ProseMirror, and CodeMirror apps, not just local
   fixtures.
4. Polish model install cancel/retry/offline states after the new in-app
   install/repair flow.
