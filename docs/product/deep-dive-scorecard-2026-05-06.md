...
and Claude desktop layouts. Current-head Obsidian long-note proof now passes. The latest
...
| Obsidian support | 9.5/10 | Default, non-default theme, split/side pane, and current-head long scrolled-note lanes have recorded strict visual proof with two verified accepts each in the disposable proof vault. The long-note lane now uses the CodeMirror viewport-end repair and records strict screenshot evidence at `2026-05-13T02:53:49Z`. The harness seeds the proof note before opening Obsidian, keeps user vaults out of scope, resets pane variants between accepts when needed, and uses exact AX value replacement plus same-pid descendant verification so CodeMirror focus churn does not turn a good insertion into a false failure. Broader vault layouts and hidden-caret edge cases remain useful follow-up proof. |
...
| Real-app smoke | 9.25/10 | TextEdit, Apple Notes title/body/checklist, core Chrome fixtures, public Chrome textarea/contenteditable, real Chrome Monaco/ProseMirror under isolated renderer-accessibility mode, normal Chrome default-AX ProseMirror, Chrome chat-like no-submit, Obsidian default/theme/pane/long-note, Codex one-word no-submit, Claude Code default Terminal-host no-submit, and Claude desktop default composer all have bounded recorded proof rows. `./script/manual_smoke_status.sh --strict` now requires 29 current-head proof refreshes after the latest app-code source change. Normal-Chrome Monaco and Claude desktop layout variants remain honest current-head proof gaps. |
