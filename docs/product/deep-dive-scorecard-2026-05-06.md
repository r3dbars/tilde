...
| TextEdit support | 9.8/10 | Fresh bounded screenshot-backed run at 2026-05-09T05:58:04Z on `986a50ed` shows ghost text aligned after the caret with two verified accepts and current proof fingerprints. The smoke harness respects the active full-accept shortcut, opens a unique disposable file through a fresh TextEdit instance, targets that exact AX window/title even when old TextEdit windows are restored, seeds only that disposable text area, and dismisses TextEdit's native inline completion before waiting for Autocomplete Lab. |
...
| Chrome Monaco-like support | 9.55/10 | Fresh local Monaco-like proof verifies insertion, and `monaco-real` has current isolated renderer-accessibility proof on `986a50ed`. The current default-Chrome retry failed closed because normal Chrome exposed only browser chrome through AX, not the page editor content. Production editor variants remain open. |
| Chrome ProseMirror-like support | 9.6/10 | Fresh local ProseMirror-like proof verifies insertion, and `prosemirror-real` has current isolated renderer-accessibility proof on `986a50ed`. The current default-Chrome retry failed closed because normal Chrome exposed only browser chrome through AX, not the page editor content. Production editor variants remain open. |
...
| Obsidian support | 8.8/10 | Current PR #35 live proof now repairs the hidden CodeMirror spacer rows that caused false `middleOfLine` blocking and shows suggestions again on `qwen3-0.6b`. The actual Tab accept handler also fired in Obsidian, but the same synthetic slice failed post-insert verification after CodeMirror moved focus to an `AXWebArea`/new-tab surface. The last fully verified two-accept Obsidian row is stale, so default Obsidian and the `obsidian-theme`, `obsidian-pane`, and `obsidian-long-note` lanes remain honest proof gaps until a clean bounded visual two-accept pass lands. |
| Codex support | 9.1/10 | Fresh current PR #35 disposable prompt proof on `986a50ed` shows visible inline placement, one verified Tab insertion, and prompt no-submit confirmation in the same bounded slice. The helper seeds only disposable `AUTOCOMPLETE_LAB_CODEX_PROOF` text, rechecks the marked composer before accepting, confirms the marker remains after Tab, and never presses Enter. Full accept stays disabled until separate full-accept no-submit proof exists, and more prompt layouts remain open. |
...
| Real-app smoke | 9.85/10 | TextEdit, Apple Notes title/body/checklist, core Chrome fixtures, public Chrome textarea/contenteditable, real Chrome Monaco/ProseMirror under isolated renderer-accessibility mode, Chrome chat-like no-submit, and Codex one-word no-submit are green on current PR #35 proof rows. Default-Chrome real Monaco/ProseMirror was retried and still fails closed because normal Chrome exposes only browser chrome through AX. Obsidian acceptance/variants, Claude Code, and Claude desktop layout lanes remain honest proof gaps. |
...
| Obsidian | 8.8/10 | [obsidian.png](visual-placement-screenshots/obsidian.png) | Current live proof shows the ghost beside the typed word again after repairing hidden CodeMirror spacer rows. Tab reached Autocomplete Lab's accept handler, but verification failed after Obsidian changed the focused AX surface. | Pending: record a fresh default Obsidian two-accept visual row, then run `obsidian-theme`, `obsidian-pane`, and `obsidian-long-note` with strict screenshot-backed trace rows. |
