...
| Real-app smoke | 9.92/10 | TextEdit, Apple Notes title/body/checklist, core Chrome fixtures, public Chrome textarea/contenteditable, real Chrome Monaco/ProseMirror under isolated renderer-accessibility mode and normal Chrome default AX, Chrome chat-like no-submit, defined Obsidian lanes, and Codex one-word no-submit are green on current proof rows. Claude Code host variants and Claude desktop layout lanes remain honest current-head proof gaps. |
