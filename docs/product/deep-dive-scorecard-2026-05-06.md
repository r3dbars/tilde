...
| Non-annoyance | 8.9/10 | Esc, typed-over tracking, repetition suppression, visible placement-uncertainty quiet mode, pause control, insertion recovery, app-specific AX cooldowns, and Settings "why hidden" copy help, but visual misses still make the app feel annoying when real-app placement proof is missing. |
| Privacy | 9.8/10 | Local-first, secure fields suppressed, password/token/API-key-like fingerprints blocked before text reads, diagnostics redact text by default, screenshots are opt-in, recent word memory is app-scoped, raw/global/per-app screenshot debug capture expires from app UI, permission copy states what is read and why, and Diagnostics exports a redacted privacy bundle with a manifest and checklist. Private-beta feedback still needs a no-raw-default workflow. |
| Onboarding | 9.1/10 | Settings explains runtime readiness, current app state, local privacy controls, and Accessibility setup in one short paragraph. Screen Recording copy appears only when screenshot capture is enabled, first success points to TextEdit, and missing/invalid app-owned MLX assets can now install or repair in-app with progress, cancel, retry, validation, and runtime warm. Default app enablement and plainer offline/model-host recovery copy remain open. |
...
3. Test real production Monaco, ProseMirror, and CodeMirror apps, not just local
   fixtures.
4. Polish default app enablement and offline/model-host recovery copy after
   the new in-app model install/repair flow.
