...
| User control | 9.6/10 | Pause, current-field/session silence, current-app enablement, visible per-app render mode, force-mirror override, green/yellow/diagnostics-only/unsupported support status, privacy controls, temporary screenshot/raw trace toggles, local log deletion, full-accept shortcut state, and Settings "why hidden" copy are clearer; full shortcut editing is still thin. |
