...
| Claude Code | 9.5/10 | [claude-code-terminal.png](visual-placement-screenshots/claude-code-terminal.png) | Recorded no-submit smoke at 2026-05-12T06:41:56Z on `ca41cdc33e5e` proves a one-word Tab accept in the real Terminal-hosted Claude Code prompt without shell or agent submit; the row is insertion-only because the fresh signed test app hit a macOS screen/audio prompt that was not granted. Later strict Terminal-host rows exist, but current-head refresh is still required. The adapter stays proof-only, marker-gated, shell-command guarded, active-output guarded, stale-focus rechecked, host-labelable, full-accept disabled, and now handles current marked prompt lines even when Terminal scrollback contains prior shell commands. | Pending: current-head Terminal refresh, host-labeled iTerm2 and Ghostty proof rows on this Mac; Warp is not installed here; separate full-accept no-submit proof stays blocked. |
