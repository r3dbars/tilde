...
| TextEdit support | 9.6/10 | Current strict visual proof covers default, multiline, and wrapped-line TextEdit runs with two verified accepts each. The smoke harness now respects the active full-accept shortcut and avoids stale warmup/runtime noise after verified accepts. |
...
| Automated tests | 10/10 | `swift test` passes 514 tests, including app-target settings state tests, diagnostics typing-health tests, scoped recent-word memory, privacy expiry, support status, serial AX reader, focused AX-health cooldown, focused-poll backoff, dogfood false-positive coverage, neutral word-completion vocabulary, screenshot trace capture policy, placement trust policy, and trace visual evidence. Script self-tests now cover strict score targets, the score loop path, manual smoke status, visual proof, typing performance, real-app smoke planning, and the endurance soak command. |
| Real-app smoke | 8.9/10 | TextEdit default/multiline/wrapped and Chrome textarea/contenteditable/editor-like/Monaco-like/ProseMirror-like/chat-like are green on the current build with strict visual evidence. Notes title/body/checklist, Obsidian fresh visual proof, Codex one-word no-submit, Claude Code, and Claude desktop remain honest manual-gated gaps. |
...
- Current real-app proof loop: `AUTOCOMPLETE_LAB_SCREENSHOT_TRACE=1 ./script/real_app_smoke.sh textedit`, `textedit-multiline`, `textedit-wrapped`, and `AUTOCOMPLETE_LAB_SCREENSHOT_TRACE=1 ./script/real_app_smoke.sh chrome --fixture all` passed with strict visual trace evidence and two verified accepts per full-accept surface. The Chrome chat-like fixture also kept submit count at zero.
- `./script/manual_smoke_status.sh --strict`: exits 1 honestly. Current TextEdit and Chrome rows pass; remaining insertion gaps are Notes title/body/checklist, Codex, Claude Code, and Claude desktop; remaining screenshot gaps are Obsidian, Notes title/body/checklist, Claude Code, and Claude desktop; Codex still needs same-slice one-word no-submit visual proof.
- `./script/smoke_test.sh`: passed after the manual-smoke readiness gate change. The run built the app, passed 514 Swift tests, passed model asset verification, package preflight, script self-tests, visual evidence checks, trace eval checks, typing-performance checks, app signing/verification, and diagnostics verification.
