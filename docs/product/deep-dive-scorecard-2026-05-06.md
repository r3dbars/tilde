...
| Notes support | 7.5/10 | Profile is safer than before, and the title field now has bounded strict visual proof with two verified accepts. Notes now tries verified AX insertion before key fallback while keeping strict post-insert verification. Body and checklist remain separate proof targets; the old key-event-only body run verified that synthetic key insertion can report success while the AX text stays unchanged, so those rows stay open. |
