...
| Real-app smoke | 9.9/10 | TextEdit, Apple Notes title/body/checklist, core Chrome fixtures, public Chrome textarea/contenteditable, real Chrome Monaco/ProseMirror under isolated renderer-accessibility mode and normal Chrome default AX, Chrome chat-like no-submit, and Codex one-word no-submit are green on current proof rows. Default Obsidian variants, Claude Code host variants, and Claude desktop layout lanes remain honest current-head proof gaps. |
