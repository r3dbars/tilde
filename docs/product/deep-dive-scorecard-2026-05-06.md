...
| Codex support | 9.1/10 | Recorded strict Codex proof shows visible inline placement, one-word Tab accept, verified insertion in the marked composer, and no prompt submit in the same bounded trace slice. It now needs current-head refresh after the latest app-code source change. The smoke lane backs up and restores an existing Codex draft. Full accept stays disabled until separate full-accept no-submit proof exists, and more prompt layouts remain open. |
...
- `./script/manual_smoke_status.sh --strict`: exits 1 honestly on remaining
  ...
  no-submit proof, while full accept remains disabled until a separate
  full-accept no-submit lane exists.
