...
- Chrome proof-runner pass: `swift test --filter AppProofCommandRunnerTests`
  and `./script/real_app_smoke_self_test.sh` passed after the Settings Chrome
  proof command started running the forced all-fixtures lane plus default-AX
  real Monaco and real ProseMirror add-on lanes.
