...
| Acceptance reliability | 9.85/10 | TextEdit, core Chrome fixtures, Chrome chat-like, public Chrome text fields, normal-Chrome real Monaco/ProseMirror default-AX rows, Apple Notes title/body/checklist, defined Obsidian lanes, and Codex one-word no-submit verify accept paths on current proof rows. Prompt-app full accept is intentionally disabled until separate full-accept no-submit proof exists, and a profile-aware safety gate blocks no-submit profiles from full accept, multiword accepted text, non-visible accepted text, and newline/tab/control accepted text before insertion. Selected text is blocked before suggestions/acceptance, AX insertion is faster, full accept has trace-safe exact-visible-text proof, Tab accept is traced as a visible-prefix accept, after-cursor drift fails verification, and missing/changed post-write verification targets are recorded as insertion failures instead of disappearing. Claude Code host variants and Claude desktop layout lanes remain honest proof gaps. |
| Visual caret alignment | 9.88/10 | TextEdit, core Chrome fixtures, Chrome chat-like, public Chrome text fields, real Monaco/ProseMirror under isolated forced-renderer Chrome and normal Chrome default AX, Apple Notes title/body/checklist, defined Obsidian lanes, and Codex have current screenshot-backed proof. Stale line rects are dropped, vertical clipping is enforced, too-narrow inline space suppresses display instead of showing a sliver, async suggestions refresh current geometry before display, and low-confidence mirror fallback is now suppressed for untrusted yellow profiles instead of showing detached element/window ghosts. More prompt-app layouts still need strict rows. |
...
| Real-app smoke | 9.92/10 | TextEdit, Apple Notes title/body/checklist, core Chrome fixtures, public Chrome textarea/contenteditable, real Chrome Monaco/ProseMirror under isolated renderer-accessibility mode and normal Chrome default AX, Chrome chat-like no-submit, defined Obsidian lanes, Codex one-word no-submit, and Claude Code default Terminal-host no-submit all have bounded proof rows. Claude Code strict screenshot refresh, Claude Code host variants, and Claude desktop layout lanes remain honest current-head proof gaps. |
