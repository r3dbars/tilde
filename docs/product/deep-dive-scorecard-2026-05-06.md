...
- Chrome smoke safety hardening: the real-app smoke script now refuses concurrent proof runs through a local lock and checks that Chrome is frontmost with the expected active tab URL before sending Chrome smoke keystrokes. This was added after an interrupted production-proof attempt exposed stale concurrent smoke processes as a wrong-app typing risk. Scores remain unchanged until a clean official-demo proof run completes.
