...
This is the live scorecard for the tilde build after the latest
...
the harness reasserted the disposable scrolled note target after Tilde
launch.
...
It is not a 10/10 yet. The 10 boring beta-safe writing rows now have current
strict proof, but that is still a narrow claim: TextEdit, Notes, Obsidian, and
local Chrome fixtures. Prompt apps, terminal hosts, chat surfaces, Mail, public
browser pages, and production browser apps remain proof-only or blocked unless
exact current proof exists.
...
| Acceptance reliability | 9.6/10 | `./script/manual_smoke_status.sh --strict` passes on the current checkout with TextEdit, Chrome local textarea/contenteditable fixtures, Apple Notes title/body/checklist, and Obsidian default/theme/pane/long-note all current. Codex default-composer full accept has a separate bounded no-submit proof, while broader prompt/chat/terminal/browser-production lanes remain proof-only or blocked and do not count as beta-safe support. Selected text is blocked before suggestions/acceptance, AX insertion is faster, full accept has trace-safe exact-visible-text proof, Tab accept is traced as a visible-prefix accept, after-cursor drift fails verification, and missing/changed post-write verification targets are recorded as insertion failures instead of disappearing. |
| Visual caret alignment | 9.55/10 | `./script/manual_smoke_status.sh --strict` passes on the current checkout and confirms strict visual trace evidence for TextEdit, Chrome local textarea/contenteditable fixtures, Apple Notes title/body/checklist, and Obsidian default/theme/pane/long-note. Stale line rects are dropped, vertical clipping is enforced, too-narrow inline space suppresses display instead of showing a sliver, async suggestions refresh current geometry before display, and low-confidence mirror fallback is now suppressed for untrusted yellow profiles instead of showing detached element/window ghosts. Prompt/editor/chat/terminal lanes remain proof-only or blocked. |
...
| Screenshot tracing | 10/10 | Screen Recording is preflighted, capture runs off the hot path, screenshots include editor bounds plus ghost text, traces/logs include capture rect plus rendered panel rect, and capture has a backlog guard plus timeout. Fresh stable-build TextEdit, forced-Chrome real-editor, and normal-Chrome default-AX ProseMirror traces prove screenshot-backed placement without raw text or screenshot-path leakage in the report. |
| TextEdit support | 9.8/10 | Fresh bounded screenshot-backed run at 2026-05-28T14:34:38Z on `f2c9f1775460` shows ghost text aligned after the caret with two verified accepts, strict visual trace evidence, and current proof fingerprints. The smoke harness now reasserts the disposable TextEdit window before undo proof and performs rollback proof before waiting on screenshot evidence, so focus drift and expired undo windows fail less ambiguously. |
| Notes support | 9.4/10 | Title, body, and checklist all have current bounded strict visual proof with two verified accepts each. Notes now tries verified AX insertion before key fallback, uses a slower read-only verification recheck for Notes AX lag, repairs stale Notes text-after-cursor reads before display/verification, and still fails closed if accepted text is unchanged. The typed-over cooldown was shortened from 5s to 750ms, and cooldown expiry now re-arms the same snapshot so suggestions recover without another keystroke. More list lengths, checked items, and undo variants are still needed. |
| Chrome textarea support | 9.5/10 | Local textarea fixture proof passed at 2026-05-28T14:40:02Z on `e2da385b0ece` with two verified accepts and strict visual trace evidence. Public pages and production browser apps no longer count toward beta-safe Chrome support. |
| Chrome contenteditable support | 9.6/10 | Local contenteditable fixture proof passed at 2026-05-28T14:41:04Z on `e2da385b0ece` with two verified accepts and strict visual trace evidence. Public rich-text pages and production browser apps no longer count toward beta-safe Chrome support. |
| Chrome editor-like support | 9/10 | Fresh full-frame screenshot and two verified accepts. |
| Chrome Monaco-like support | 9.5/10 | Fresh local Monaco-like proof verifies insertion, and `monaco-real` has current isolated renderer-accessibility proof with two verified accepts. Normal-Chrome default AX Monaco is pending because current reruns expose too little verified text context and setup verification stays unchanged. Production editor variants remain open. |
| Chrome ProseMirror-like support | 9.7/10 | Fresh local ProseMirror-like proof verifies insertion, `prosemirror-real` has current isolated renderer-accessibility proof, and `prosemirror-real-default` now has current normal-Chrome default-AX proof at 2026-05-12T09:19:46Z with two verified accepts and strict visual evidence. Production editor variants remain open. |
...
| Obsidian support | 9.5/10 | Default, non-default theme, split/side pane, and long scrolled-note lanes have current strict visual proof with two verified accepts each in the disposable proof vault. The current refresh includes default/theme/pane rows plus a long-note pass at 2026-05-28T16:56:55Z on `1969992ddcf5` after the harness reasserted the disposable scrolled-note target following Tilde launch. The harness seeds the proof note before opening Obsidian, keeps user vaults out of scope, resets pane variants between accepts when needed, retries pane setup, and uses exact AX value replacement plus same-pid descendant verification so CodeMirror focus churn does not turn a good insertion into a false failure. Broader vault layouts and hidden-caret edge cases remain useful follow-up proof. |
| Codex support | 9.1/10 | Recorded strict Codex proof shows visible inline placement, one-word Tab accept, verified insertion in the marked composer, and no prompt submit in one bounded trace slice. A separate default-composer full-accept no-submit proof is recorded and validated by `./script/check_prompt_app_manifest_proof.sh`. The smoke lane backs up and restores an existing Codex draft. More prompt layouts remain open before Codex can rise beyond proof-only/default-composer support. |
| Claude Code support | 9.6/10 | Direct bundle support is still diagnostics-only because `com.anthropic.claude-code` is a background-only CLI helper on this machine. A proof-only terminal-host adapter maps supported terminal hosts to a virtual Claude Code profile only when proof mode is active, validates the disposable marker/current input line/shell-prompt guard, rejects command-shaped prompt lines, active agent output, stale marked scrollback, and multiline command buffers, maps the verified proof prompt back to a compose field for suggestions, carries the virtual bundle ID through suggestion traces, rechecks focus and the acceptance snapshot before insertion, accepts one word from phrase-mode suggestions, tries verified hardware, Unicode, and proof-only paste insertion, and keeps full accept disabled. Current bounded strict visual smoke proves Terminal at 2026-05-25T13:56:48Z and iTerm2 at 2026-05-25T15:34:16Z with verified one-word Tab accepts and no shell or agent submit. Ghostty reached visible phrase presentation and Tab capture at 2026-05-25T16:12:56Z, but diagnostics lines 375775-375824 show insertion failed closed after hardware key events, Unicode key events, and verified paste all left the prompt unchanged. Warp is not installed here. |
| Claude desktop support | 9.45/10 | Current strict visual smoke at 2026-05-12T11:12:24Z proves same-baseline synthetic-caret placement, exactly one verified Tab one-word accept, and no prompt submit signal in Claude desktop. A live failure exposed that generic AX insertion could miss the composer even when placement and key capture were correct, so Claude desktop now uses a prompt-bound direct AX value replacement path and suppresses after insertion failure. Full accept stays disabled until separate full-accept no-submit proof exists, and more prompt layouts still need coverage. |
...
| Real-app smoke | 9.25/10 | TextEdit, Apple Notes title/body/checklist, Chrome local textarea/contenteditable fixtures, and Obsidian default/theme/pane/long-note are current and pass `./script/manual_smoke_status.sh --strict` on this checkout. Prompt, chat, terminal-host, public browser, and production browser rows remain proof-only or blocked. |
| Release readiness | 8/10 | Packaging is in decent shape and the beta-safe manual/screenshot-backed proof grid, runtime no-egress proof, and private beta packet are current again, but beta readiness still correctly fails on onboarding and packaged latency. Notarization/stapling and beta onboarding still need a final product pass. |
...
| --- | ---: | --- | --- | --- |
| TextEdit | 10/10 | [textedit-inline.png](visual-placement-screenshots/textedit-inline.png), [textedit-light.png](visual-placement-screenshots/textedit-light.png), [textedit-dark.png](visual-placement-screenshots/textedit-dark.png), [textedit-long-wrap.png](visual-placement-screenshots/textedit-long-wrap.png), [textedit-narrow.png](visual-placement-screenshots/textedit-narrow.png), [textedit-scrolled.png](visual-placement-screenshots/textedit-scrolled.png) | Ghost is on the same line, after the caret, readable, and not focus-stealing across default, light, dark, wrapped, narrow, and scrolled TextEdit documents. | Rerun secondary-display proof when a second display is connected. |
...
| Chrome contenteditable | 9.6/10 | [chrome-contenteditable.png](visual-placement-screenshots/chrome-contenteditable.png) | Local contenteditable proof is current, the public MediumEditor-style lane is current, and the ghost starts immediately after the caret with enough contrast. | Pending: broaden to more public rich-text/editor domains. |
| Chrome editor-like | 9/10 | [chrome-editor-like.png](visual-placement-screenshots/chrome-editor-like.png), [chrome-codemirror-official.png](visual-placement-screenshots/chrome-codemirror-official.png) | CodeMirror-style fixture aligns well after the caret, and the committed official CodeMirror screenshot remains useful historical placement evidence. | Pending: rerun the official CodeMirror lane after Tilde Accessibility is enabled and keep production editor variants honest. |
| Chrome Monaco-like | 9.5/10 | [chrome-monaco-like.png](visual-placement-screenshots/chrome-monaco-like.png), [chrome-monaco-real.png](visual-placement-screenshots/chrome-monaco-real.png) | Local Monaco-like proof is readable near the caret, and real Monaco has recorded strict screenshot evidence in isolated forced-renderer Chrome. | Pending: current-head refresh, normal Chrome default AX, and broader production editor variants. |
| Chrome ProseMirror-like | 9.75/10 | [chrome-prosemirror-like.png](visual-placement-screenshots/chrome-prosemirror-like.png), [chrome-prosemirror-real.png](visual-placement-screenshots/chrome-prosemirror-real.png) | Local ProseMirror-like proof is inline, and real ProseMirror has recorded strict screenshot evidence in isolated forced-renderer Chrome plus normal Chrome default AX. | Pending: current-head refresh and broader production editor variants. |
...
| Obsidian | 9.5/10 | [obsidian.png](visual-placement-screenshots/obsidian.png) | Default, non-default theme, pane, and long-note Obsidian lanes have recorded strict visual rows with two verified accepts each. Direct AX value replacement plus descendant verification handles CodeMirror stale cursor/focus churn without changing models. | Pending: broader vault layouts and hidden-caret edge cases. |
| Codex | 10/10 | [codex-inline.png](visual-placement-screenshots/codex-inline.png) | Bounded strict smoke shows the ghost visible on the same line after the caret, a one-word Tab accept, verified insertion, and no prompt submit in one trace slice. Separate bounded full-accept proof also verifies no-submit behavior for the default composer. | Pending: more prompt layouts before raising beyond proof-only/default-composer support. |
...
| Claude Code | 9.75/10 | [claude-code-terminal.png](visual-placement-screenshots/claude-code-terminal.png) | Current bounded strict visual smoke proves inline Terminal-host placement at 2026-05-25T13:56:48Z and inline iTerm2-host placement at 2026-05-25T15:34:16Z, each with one verified Tab one-word accept from a phrase-mode suggestion and no shell or agent submit. The iTerm2 row records diagnostics lines 372371-372550, trace lines 24920-24930, visual `strict-complete`, app proof `app-sha256:c6811e3591a05730aac82cb2c0b2eaa625c28b2f067de1f7fd187cfc27870731`, and archive proof `archive-sha256:6c241fb4ffdf57cfe825818bd4f52a3871181d409328f7ec6350f667f6700387`. The adapter stays proof-only, marker-gated, shell-command guarded, active-output guarded, stale-focus rechecked, host-labelable, verified-insertion-only, and full-accept disabled. Ghostty displayed a valid phrase and consumed Tab, but failed closed at insertion, so it is not claimed. | Pending: a Ghostty-specific insertion path or explicit unsupported-host status; Warp is not installed here; separate full-accept no-submit proof stays blocked. |
| Claude desktop | 9.45/10 | [claude-desktop.png](visual-placement-screenshots/claude-desktop.png) | Bounded strict visual smoke at 2026-05-12T11:12:24Z proves same-baseline ghost placement plus a one-word Tab accept in the real Claude desktop composer without a submit signal, and the current app/archive fingerprint still matches that row. Claude desktop now uses direct prompt-bound AX value replacement after generic AX insertion failed live. | Pending: record empty, long, wrapped, narrow, context, light, and dark layout rows; full accept remains disabled because no safe full-accept no-submit proof command exists yet. |

## Latest Proof

- 2026-05-25 Claude Code terminal-host pass: `claude-code-iterm2`
  recorded a strict one-word no-submit row, while `claude-code-ghostty`
  reached visible phrase placement and Tab capture but failed closed when all
  verified insertion paths left the Ghostty prompt unchanged.
...
- Chrome proof-runner pass: `swift test --filter AppProofCommandRunnerTests`
  ...
  all-fixtures lane plus default-AX real-editor add-on lanes. The current
  proof only claims default-AX ProseMirror; Monaco remains pending after
  setup verification failed. Failed proof command starts now clean up temporary
  proof mode.
...
- Prompt-app safety worker pass: full `swift test` passed 318 tests after
  keeping prompt-app full accept proof-gated and suppressing unsafe prompt
  actions like Enter/send/submit/run.
...
- `./script/manual_smoke_status.sh --strict`: exits 1 honestly on remaining
  non-Codex proof gaps. Codex now has same-slice strict visual one-word
  no-submit proof plus a separate bounded full-accept no-submit lane for the
  default composer.
...
- `AUTOCOMPLETE_LAB_SCREENSHOT_TRACE=1 script/real_app_smoke.sh claude-code-terminal --manual-gate`: now opens the disposable Terminal-hosted Claude Code prompt itself, presses Tab once, falls back from ignored Unicode key events to verified hardware key events without touching pasteboard, and records the current strict visual no-submit row at 2026-05-25T13:56:48Z.
...
- `./script/manual_smoke_status.sh --strict`: now shows Chrome chat-like no-submit, Obsidian, Notes title/body/checklist, Codex, Claude Code, and Claude desktop as passed for their bounded rows, and still fails honestly on remaining score targets and variant gaps.
...
- `./script/check_visual_placement_evidence.sh`: passed with verified visual-placement screenshots, including the Codex same-slice row.
- `./script/check_visual_placement_evidence.sh --require-all`: still fails honestly on remaining non-Codex proof gaps until every required variant row is closed.
...
- `AUTOCOMPLETE_LAB_SCREENSHOT_TRACE=1 ./script/real_app_smoke.sh chrome --fixture monaco-real --skip-build`: passed at 2026-05-08T03:19:38Z with pinned upstream Monaco, isolated temp-profile Chrome, renderer accessibility forced, strict screenshot evidence, Tab accept, Option-Tab full accept, and two verified insertions. Monaco's own suggestions are disabled in the fixture so the proof targets Tilde, not Monaco's completion menu.
- Historical default-Chrome real-editor proof from 2026-05-08 is no longer treated as current. The 2026-05-12 follow-up found the old default-AX Monaco proof was too weak: after clearing the stale isolated DevTools port and avoiding coordinate refocus clicks, `prosemirror-real-default` passed at 2026-05-12T09:19:46Z on pushed commit `46c9dbd21a86`, but `monaco-real-default` still failed because normal Chrome AX exposed a one-character text area and setup verification stayed unchanged.
- Chrome production-proof harness pass: `bash -n script/real_app_smoke.sh script/manual_smoke_session.sh script/real_app_smoke_self_test.sh`, `./script/real_app_smoke_self_test.sh`, and `./script/check_proof_manifest.sh` passed after adding `codemirror-official`, `monaco-official`, and `prosemirror-official` dry-run lanes. CodeMirror and ProseMirror now have bounded official-demo traces; Monaco remains capped until its official-demo trace passes.
- Chrome smoke safety hardening: the real-app smoke script now refuses concurrent proof runs through a local lock plus an active-process scan, checks that Chrome is frontmost with the expected active tab URL, uses isolated temporary Chrome profiles plus localhost DevTools for official-demo readiness/focus/setup, requires a focused editable web text AX target, and sends Chrome setup text to the Chrome process with AX/value or DevTools verification instead of global setup keystrokes. `prosemirror-official` now has strict screenshot-backed proof from 2026-05-12T07:47:10Z, and `codemirror-official` has strict screenshot-backed proof from 2026-05-12T08:14:38Z after preloading the disposable CodeMirror document and repairing its official-page AX soft-wrap cursor drift. Monaco still fails honestly on current-page AX context, so scores remain capped until that remaining official-demo trace passes.
...
1. Expand prompt-app proof beyond the current one-word no-submit rows. Codex,
   Claude Code, and Claude desktop now have bounded one-word no-submit proof,
   but still need more prompt-layout and full-accept no-submit coverage before
   broad prompt support can be considered done.
