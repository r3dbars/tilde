...
| Real-app smoke | 9.86/10 | TextEdit, Apple Notes title/body/checklist, core Chrome fixtures, public Chrome textarea/contenteditable, real Chrome Monaco/ProseMirror under isolated renderer-accessibility mode, normal Chrome default-AX ProseMirror, Chrome chat-like no-submit, defined Obsidian lanes, Codex one-word no-submit, and Claude Code default Terminal-host no-submit all have bounded proof rows. Normal-Chrome Monaco, Claude Code strict screenshot refresh, Claude Code host variants, and Claude desktop layout lanes remain honest current-head proof gaps. |
