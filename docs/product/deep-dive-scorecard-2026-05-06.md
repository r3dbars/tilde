...
It is not a 10/10 yet. The biggest remaining gap is current strict proof for
the boring beta-safe rows: TextEdit, Notes, Obsidian, and Chrome local
textarea/contenteditable fixtures. Prompt apps, terminal hosts, chat surfaces,
Mail, public browser pages, and production browser apps remain proof-only or
blocked unless exact current proof exists.
...
| TextEdit support | 9.8/10 | Fresh bounded screenshot-backed run at 2026-05-09T07:36:50Z on `8928086dca3a` shows ghost text aligned after the caret with two verified accepts and current proof fingerprints. The smoke harness respects the active full-accept shortcut, opens a unique disposable file through a fresh TextEdit instance, targets that exact AX window/title even when old TextEdit windows are restored, seeds only that disposable text area, and dismisses TextEdit's native inline completion before waiting for Autocomplete Lab. |
...
| Chrome textarea support | 9.5/10 | Local textarea fixture proof is the beta-safe claim. Public pages and production browser apps no longer count toward beta-safe Chrome support. |
| Chrome contenteditable support | 9.6/10 | Local contenteditable fixture proof is the beta-safe claim. Public rich-text pages and production browser apps no longer count toward beta-safe Chrome support. |
...
| Obsidian support | 9.5/10 | Default, non-default theme, split/side pane, and long scrolled-note lanes have recorded strict visual proof with two verified accepts each in the disposable proof vault. The strict gate still decides whether those rows are current for the active checkout. The harness seeds the proof note before opening Obsidian, keeps user vaults out of scope, resets pane variants between accepts when needed, and uses exact AX value replacement plus same-pid descendant verification so CodeMirror focus churn does not turn a good insertion into a false failure. Broader vault layouts and hidden-caret edge cases remain useful follow-up proof. |
...
| Real-app smoke | 9.25/10 | TextEdit, Apple Notes title/body/checklist, Chrome local textarea/contenteditable fixtures, and Obsidian default/theme/pane/long-note are the beta-safe proof rows. `./script/manual_smoke_status.sh --strict` now requires current-head proof for those 10 rows only. Prompt, chat, terminal-host, public browser, and production browser rows remain proof-only or blocked. |
