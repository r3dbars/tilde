...
| Acceptance reliability | 9.75/10 | TextEdit, core Chrome fixtures, Chrome chat-like, public Chrome textarea, isolated real Monaco/ProseMirror rows, normal-Chrome real ProseMirror default-AX rows, Apple Notes title/body/checklist, defined Obsidian lanes, and Codex one-word no-submit verify accept paths on current proof rows. Prompt-app full accept is intentionally disabled until separate full-accept no-submit proof exists, and a profile-aware safety gate blocks no-submit profiles from full accept, multiword accepted text, non-visible accepted text, and newline/tab/control accepted text before insertion. Selected text is blocked before suggestions/acceptance, AX insertion is faster, full accept has trace-safe exact-visible-text proof, Tab accept is traced as a visible-prefix accept, after-cursor drift fails verification, and missing/changed post-write verification targets are recorded as insertion failures instead of disappearing. Public contenteditable, normal-Chrome Monaco, Claude Code host variants, and Claude desktop layout lanes remain honest proof gaps. |
| Visual caret alignment | 9.8/10 | TextEdit, core Chrome fixtures, Chrome chat-like, public Chrome textarea, real Monaco/ProseMirror under isolated forced-renderer Chrome, normal Chrome default-AX ProseMirror, Apple Notes title/body/checklist, defined Obsidian lanes, and Codex have current screenshot-backed proof. Stale line rects are dropped, vertical clipping is enforced, too-narrow inline space suppresses display instead of showing a sliver, async suggestions refresh current geometry before display, and low-confidence mirror fallback is now suppressed for untrusted yellow profiles instead of showing detached element/window ghosts. Public contenteditable, normal-Chrome Monaco, and more prompt-app layouts still need strict rows. |
...
| Chrome contenteditable support | 9.3/10 | Fresh local full-frame screenshot and two verified accepts are current, but the public MediumEditor-style lane is stale after the current rerun could not keep a stable web-backed AX editor target. The verifier still tolerates Chromium rich-editor height reflow without ignoring target movement. A stable public contenteditable proof target is needed before this can return to 9.5+. |
...
| Real-app smoke | 9.84/10 | TextEdit, Apple Notes title/body/checklist, core Chrome fixtures, public Chrome textarea, real Chrome Monaco/ProseMirror under isolated renderer-accessibility mode, normal Chrome default-AX ProseMirror, Chrome chat-like no-submit, defined Obsidian lanes, Codex one-word no-submit, and Claude Code default Terminal-host no-submit all have bounded proof rows. Public contenteditable, normal-Chrome Monaco, Claude Code strict screenshot refresh, Claude Code host variants, and Claude desktop layout lanes remain honest current-head proof gaps. |
...
| Chrome contenteditable | 9.3/10 | [chrome-contenteditable.png](visual-placement-screenshots/chrome-contenteditable.png) | Local contenteditable proof is current and the ghost starts immediately after the caret with enough contrast. | Pending: refresh public contenteditable/rich-editor proof after the MediumEditor rerun failed to keep a stable AX target. |
