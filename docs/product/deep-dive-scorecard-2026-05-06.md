...
and Claude desktop layouts plus Obsidian theme, pane, and long-note shapes. The
...
| Obsidian support | 9.75/10 | Default, non-default theme, split/side pane, and long scrolled-note lanes now have current strict visual proof with two verified accepts each in the disposable proof vault. The harness seeds the proof note before opening Obsidian, keeps user vaults out of scope, resets pane/long-note variants between accepts when needed, and uses exact AX value replacement plus same-pid descendant verification so CodeMirror focus churn does not turn a good insertion into a false failure. Broader vault layouts and hidden-caret edge cases remain useful follow-up proof. |
| Codex support | 9.3/10 | Current strict Codex proof shows visible inline placement, one-word Tab accept, verified insertion in the marked composer, and no prompt submit in the same bounded trace slice. The smoke lane backs up and restores an existing Codex draft. Full accept stays disabled until separate full-accept no-submit proof exists, and more prompt layouts remain open. |
| Claude Code support | 9.55/10 | Direct bundle support is still diagnostics-only because `com.anthropic.claude-code` is a background-only CLI helper on this machine. A proof-only terminal-host adapter maps supported terminal hosts to a virtual Claude Code profile only when proof mode is active, validates the disposable marker/current input line/shell-prompt guard, rejects command-shaped prompt lines, active agent output, stale marked scrollback, and multiline command buffers, carries the virtual bundle ID through suggestion traces, rechecks focus and the acceptance snapshot before insertion, inserts through the terminal host's own Paste menu via AX, and keeps full accept disabled. Current strict visual rows at 2026-05-12T10:58:42Z and 2026-05-12T11:04:20Z prove one verified Tab one-word accept in Terminal-hosted Claude Code without shell or agent submit. iTerm2 and Ghostty are installed but still need host-labeled proof rows; Warp is not installed here. |
...
| Real-app smoke | 9.88/10 | TextEdit, Apple Notes title/body/checklist, core Chrome fixtures, public Chrome textarea/contenteditable, real Chrome Monaco/ProseMirror under isolated renderer-accessibility mode, normal Chrome default-AX ProseMirror, Chrome chat-like no-submit, defined Obsidian lanes, Codex one-word no-submit, Claude Code default Terminal-host no-submit, and Claude desktop default composer all have bounded proof rows. Normal-Chrome Monaco, Obsidian long note, and Claude desktop layout variants remain honest current-head proof gaps. |
...
| Obsidian | 9.75/10 | [obsidian.png](visual-placement-screenshots/obsidian.png) | Default, non-default theme, pane, and long-note Obsidian lanes have current strict visual rows with two verified accepts each. Direct AX value replacement plus descendant verification handles CodeMirror stale cursor/focus churn without changing models. | Pending: broaden to more vault layouts and hidden-caret edge cases. |
