...
| Chrome Monaco-like support | 9.1/10 | Fresh local Monaco-like proof verifies insertion, and `monaco-real` now has strict screenshot proof plus two verified accepts at 2026-05-08T03:19:38Z. The real-engine lane uses isolated Chrome with renderer accessibility forced and still falls back to mirror/element placement. The new default-Chrome proof lane failed before presentation because normal Chrome exposed no editable field, so default Chrome AX exposure and caret-quality placement remain open. |
| Chrome ProseMirror-like support | 9.2/10 | Fresh local ProseMirror-like proof verifies insertion, and `prosemirror-real` now has strict screenshot proof plus two verified accepts at 2026-05-08T03:17:54Z. The real-engine lane uses isolated Chrome with renderer accessibility forced and still falls back to mirror/element placement. The new default-Chrome proof lane failed before presentation because normal Chrome exposed no editable field, so default Chrome AX exposure and caret-quality placement remain open. |
...
| Chrome Monaco-like | 9.1/10 | [chrome-monaco-like.png](visual-placement-screenshots/chrome-monaco-like.png), [chrome-monaco-real.png](visual-placement-screenshots/chrome-monaco-real.png) | Local Monaco-like proof is readable near the caret, and real Monaco now has strict screenshot evidence with Tab and Option-Tab verified. | Pending: default Chrome AX exposure and caret-quality real Monaco placement; current real-engine proof uses forced renderer accessibility and mirror/element fallback. |
| Chrome ProseMirror-like | 9.2/10 | [chrome-prosemirror-like.png](visual-placement-screenshots/chrome-prosemirror-like.png), [chrome-prosemirror-real.png](visual-placement-screenshots/chrome-prosemirror-real.png) | Local ProseMirror-like proof is inline, and real ProseMirror now has strict screenshot evidence with Tab and Option-Tab verified. | Pending: default Chrome AX exposure and caret-quality real ProseMirror placement; current real-engine proof uses forced renderer accessibility and mirror/element fallback. |
...
- `AUTOCOMPLETE_LAB_SCREENSHOT_TRACE=1 ./script/real_app_smoke.sh chrome --fixture monaco-real --chrome-accessibility default`: failed on 2026-05-08 before suggestion presentation. The harness now preflights normal Chrome's AX tree and reports the precise blocker: the active window is ready, but macOS Accessibility exposes only browser chrome, not page `AXWebArea`/`AXTextArea` content.
- `AUTOCOMPLETE_LAB_SCREENSHOT_TRACE=1 ./script/real_app_smoke.sh chrome --fixture prosemirror-real --chrome-accessibility default --skip-build`: remains pending behind the same default-Chrome renderer-accessibility blocker.
