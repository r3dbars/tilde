...
| Codex support | 9.1/10 | The last disposable prompt proof on `361a62f19239` showed visible inline placement, one verified Tab insertion, and prompt no-submit confirmation in the same bounded slice. It is stale after later source changes, so Codex remains a current-head proof gap until the prompt-safe lane is rerun. The helper seeds only disposable `AUTOCOMPLETE_LAB_CODEX_PROOF` text, rechecks the marked composer before accepting, confirms the marker remains after Tab, and never presses Enter. Full accept stays disabled until separate full-accept no-submit proof exists, and more prompt layouts remain open. |
