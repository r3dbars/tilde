...
| Acceptance reliability | 9.7/10 | TextEdit, core Chrome fixtures, Chrome chat-like, public Chrome text fields, Apple Notes title/body/checklist, and Codex one-word no-submit verify accept paths on PR #35 proof rows. The latest source commit `025399a9` re-verifies Notes checklist specifically; the broader rows from `8d66322b` are now strict-status stale and need a refresh before release gating. Prompt-app full accept is intentionally disabled until separate full-accept no-submit proof exists, and a profile-aware safety gate blocks no-submit profiles from full accept, multiword accepted text, non-visible accepted text, and newline/tab/control accepted text before insertion. Selected text is blocked before suggestions/acceptance, AX insertion is faster, full accept has trace-safe exact-visible-text proof, Tab accept is traced as a visible-prefix accept, after-cursor drift fails verification, and missing/changed post-write verification targets are recorded as insertion failures instead of disappearing. Obsidian acceptance, Claude Code, Claude desktop, and default-Chrome real-editor AX lanes remain honest proof gaps. |
| Visual caret alignment | 9.8/10 | TextEdit, core Chrome fixtures, Chrome chat-like, public Chrome text fields, real Monaco/ProseMirror under isolated forced-renderer Chrome, Apple Notes title/body/checklist, Codex, Obsidian, Claude Code, and Claude desktop now have screenshot-backed proof. Stale line rects are dropped, vertical clipping is enforced, too-narrow inline space suppresses display instead of showing a sliver, async suggestions refresh current geometry before display, and low-confidence mirror fallback is now suppressed for untrusted yellow profiles instead of showing detached element/window ghosts. Current default-Chrome real-editor proof is blocked because normal Chrome exposed only browser chrome in the latest rerun, and Obsidian still needs current-build acceptance proof. |
...
| TextEdit support | 9.65/10 | Bounded screenshot-backed run at 2026-05-09T05:47:31Z on `8d66322b` shows ghost text aligned after the caret with two verified accepts. It is strict-status stale after the latest retry commit, so it needs one refresh. The smoke harness respects the active full-accept shortcut, opens a unique disposable file through a fresh TextEdit instance, targets that exact AX window/title even when old TextEdit windows are restored, seeds only that disposable text area, and dismisses TextEdit's native inline completion before waiting for Autocomplete Lab. |
| Notes support | 9.35/10 | Checklist now has current-commit `025399a9` bounded strict visual proof with two verified accepts after the prefix-cooldown retry fix. Title and body passed on `8d66322b` and are strict-status stale after the latest source commit, so they need a refresh. Notes now tries verified AX insertion before key fallback, uses a slower read-only verification recheck for Notes AX lag, repairs stale Notes text-after-cursor reads before display/verification, and still fails closed if accepted text is unchanged. The typed-over cooldown was shortened from 5s to 750ms, and cooldown expiry now re-arms the same snapshot so suggestions recover without another keystroke. More list lengths, checked items, and undo variants are still needed. |
...
| Codex support | 9/10 | The `8d66322b` disposable prompt proof shows visible inline placement, one verified Tab insertion, and prompt no-submit confirmation in the same bounded slice, but it is strict-status stale after the latest source commit. The helper seeds only disposable `AUTOCOMPLETE_LAB_CODEX_PROOF` text, rechecks the marked composer before accepting, confirms the marker remains after Tab, never presses Enter, and the disposable marker was cleared afterward. Full accept stays disabled until separate full-accept no-submit proof exists, and more prompt layouts remain open. |
...
| Real-app smoke | 9.6/10 | The latest current-commit proof is Notes checklist on `025399a9`, which passed with two accepts and strict screenshot evidence after the cooldown retry fix. TextEdit, Notes title/body, core Chrome fixtures, public Chrome textarea/contenteditable, real Chrome Monaco/ProseMirror under isolated renderer-accessibility mode, Chrome chat-like no-submit, and Codex one-word no-submit all passed minutes earlier on `8d66322b`, but strict status now marks them stale after the source retry commit. Older default-Chrome real Monaco/ProseMirror evidence is also stale because the latest default-Chrome rerun exposed only browser chrome through AX. Obsidian acceptance/variants, Claude Code, and Claude desktop layout lanes remain honest proof gaps. |
