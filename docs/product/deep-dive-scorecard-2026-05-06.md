...
- Chrome smoke safety hardening: the real-app smoke script now refuses concurrent proof runs through a local lock plus an active-process scan, checks that Chrome is frontmost with the expected active tab URL, uses isolated temporary Chrome profiles plus localhost DevTools for official-demo readiness/focus/setup, requires a focused editable web text AX target, and sends Chrome setup text to the Chrome process with AX/value or DevTools verification instead of global setup keystrokes. `prosemirror-official` now has strict screenshot-backed proof from 2026-05-12T07:47:10Z; CodeMirror and Monaco still fail honestly on current-page AX context. Scores remain capped until the remaining official-demo traces pass.
