...
placement across production-editor variants and more app layouts, especially
Chrome production editor variants, Obsidian/editor shapes, Notes variants, and
Claude desktop layouts. Codex now has the same-slice screenshot, verified
one-word accept, and no-submit proof needed to graduate from the old prompt-app
blocker, while full accept stays disabled until a separate no-submit lane exists.
...
| Chrome contenteditable support | 9/10 | Fresh local full-frame screenshot and two verified accepts. The bounded production path is now `contenteditable-public` on the public W3Schools contenteditable demo, but it still needs a strict screenshot-backed trace row before this can score as production Chrome text-field proof. |
...
| Obsidian support | 9/10 | Fresh bounded screenshot-backed run at 2026-05-07T21:15:51Z shows caret-bound synthetic mirror placement in a disposable vault note, two verified accepts, and current proof fingerprints. It now has explicit `obsidian-theme`, `obsidian-pane`, and `obsidian-long-note` proof lanes, but it stays partial until those rows are captured with bounded strict screenshot evidence. |
| Codex support | 9.3/10 | Fresh strict Codex proof at 2026-05-09T02:45:42Z shows visible inline placement, one-word Tab accept, verified insertion in the composer, and no prompt submit in the same bounded trace slice. The smoke lane backs up and restores an existing Codex draft. Full accept stays disabled until separate full-accept no-submit proof exists. |
...
| Word completion quality | 9.7/10 | Word completion and partial acceptance are useful, bounded, app-scoped, activation and fast ranking both require 3+ typed letters, fast completions obey repeated-miss suppression, and unrelated whole-word completions are rejected. `docs/evals/word-completion-quality-2026-05-09.md` now shows deterministic app-surface evidence across TextEdit, Notes, Obsidian, and Chrome-like fields: 100% candidate quality, 8% miss rate, 8% typed-over rate, 1/1 repeated-miss suppression, 1/1 prefix-family cooldown, and 4/4 partial accept success. It still needs larger live dogfood volume before this can honestly score 10/10. |
...
| Real-app smoke | 9.85/10 | TextEdit, core Chrome fixtures, real Chrome Monaco/ProseMirror under isolated renderer-accessibility mode, Chrome chat-like no-submit, and Codex one-word no-submit are green on the current stable build. Older default-Chrome real Monaco/ProseMirror evidence is now treated as stale because the latest default-Chrome rerun exposed only browser chrome through AX. Obsidian, Apple Notes title/body/checklist, Claude Code, and Claude desktop still have historical proof but need current-build refresh rows. |
...
| Chrome textarea | 9/10 | [chrome-textarea.png](visual-placement-screenshots/chrome-textarea.png) | Inline ghost is readable and follows the typed text; untrusted low-confidence mirror fallback is now suppressed instead of showing detached yellow-profile placement. | Pending: run `textarea-public` against the public W3Schools demo with strict screenshot-backed trace proof. |
| Chrome contenteditable | 9/10 | [chrome-contenteditable.png](visual-placement-screenshots/chrome-contenteditable.png) | Ghost starts immediately after the caret with enough contrast. | Pending: run `contenteditable-public` against the public W3Schools demo with strict screenshot-backed trace proof. |
...
| Chrome Monaco-like | 9.65/10 | [chrome-monaco-like.png](visual-placement-screenshots/chrome-monaco-like.png), [chrome-monaco-real.png](visual-placement-screenshots/chrome-monaco-real.png) | Local Monaco-like proof is readable near the caret, and real Monaco has current strict screenshot evidence in isolated forced-renderer Chrome. | Pending: default-Chrome AX currently exposes only browser chrome; production editor variants still need proof. |
| Chrome ProseMirror-like | 9.7/10 | [chrome-prosemirror-like.png](visual-placement-screenshots/chrome-prosemirror-like.png), [chrome-prosemirror-real.png](visual-placement-screenshots/chrome-prosemirror-real.png) | Local ProseMirror-like proof is inline, and real ProseMirror has current strict screenshot evidence in isolated forced-renderer Chrome. | Pending: default-Chrome AX currently exposes only browser chrome; production editor variants still need proof. |
...
| Obsidian | 9/10 | [obsidian.png](visual-placement-screenshots/obsidian.png) | Real CodeMirror proof now shows the ghost beside the typed word in a disposable vault note, with Tab/full accept verified in the same bounded trace slice. | Pending: run `obsidian-theme`, `obsidian-pane`, and `obsidian-long-note` with strict screenshot-backed trace rows. |
| Codex | 10/10 | [codex-inline.png](visual-placement-screenshots/codex-inline.png) | Bounded strict smoke at 2026-05-09T02:45:42Z shows the ghost visible on the same line after the caret, a one-word Tab accept, verified insertion, and no prompt submit in one trace slice. | Full accept stays disabled until separate full-accept no-submit proof exists. |
...
- `./script/manual_smoke_status.sh --strict`: exits 1 honestly on broader
  variant gaps, not on the Codex same-slice gate. Codex now has strict visual
  proof with one verified one-word no-submit accept, and Claude Code has
  terminal-host strict visual proof with one verified one-word no-submit accept.
...
- `./script/manual_smoke_status.sh --strict`: now shows Chrome chat-like no-submit, Obsidian, Notes title/body/checklist, Codex, Claude Code, and Claude desktop as passed, while still failing honestly on broader variant and score targets.
...
- `./script/check_visual_placement_evidence.sh`: passed with 16 verified visual-placement screenshots.
- `./script/check_visual_placement_evidence.sh --require-all`: passed after the Codex same-slice proof row was recorded.
...
- Historical default-Chrome real-editor proof from 2026-05-08 is no longer treated as current. The latest 2026-05-09 rerun of `AUTOCOMPLETE_LAB_SCREENSHOT_TRACE=1 ./script/real_app_smoke.sh chrome --fixture monaco-real --chrome-accessibility default --skip-build` failed before typing because normal Chrome exposed only browser chrome through macOS Accessibility, not the editor AX tree.
- Keep `monaco-real-default` and `prosemirror-real-default` pending until normal Chrome exposes editable web content again or a safer production proof path replaces those rows.
