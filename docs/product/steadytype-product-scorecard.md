...
Latest beta gate evidence checked on 2026-05-12 with the sampled default-runtime latency selector, `./script/manual_smoke_status.sh --strict`, `./script/private_beta_packet.sh --check`, and `./script/package_release.sh --check --require-notary-profile`.
...
| Beta readiness | 62/100 | `./script/beta_readiness.sh --check-only` is now stricter and requires `./script/package_release.sh --check --require-notary-profile`. `./script/package_release.sh --check --require-notary-profile` failed because `NOTARYTOOL_PROFILE` is missing. `./script/private_beta_packet.sh --check` verified the current packet. `./script/manual_smoke_status.sh --strict` still reports 30 stale or pending app proof rows. | The app has strong local gates, but private beta must stay blocked until current app proof, Apple notarization, stapling, and fresh-install Gatekeeper proof all pass. | Refresh current manual proof, add notarized install proof, rerun `./script/private_beta_packet.sh --check`, and finish with `./script/beta_readiness.sh --check-only`. |
