...
`terminal-working-directory-present`; direct `--initial-command` and `--command`
no-restore probes also failed to write a pidfile. The current blocker is
Ghostty spawning a real shell/pty before command submission. The follow-up
`20260528T090002Z-ghostty` records the new no-restore ownership path
