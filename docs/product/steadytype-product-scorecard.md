...
`retry-launch-action-enter-sent`, but still never wrote the Claude pidfile, so
the current blocker is Ghostty command execution after explicit Enter. Ghostty
remains unsupported until a detached run reaches verified one-word no-submit
insertion and exits `0`.
