...
launch, requires `preflight-finished` in the stage trace, returns exit `42` when
the bridge is unhealthy, and skips retry if launch stalls before
`new-window-start`. The current bounded preflight run
`20260528T073112Z-ghostty` exited on its own with status `42`, logged
`preflight-begin` only, printed "Ghostty AppleScript bridge preflight exited
before recording completion", skipped the remaining disposable launches, and
left both runner and smoke processes stopped.
