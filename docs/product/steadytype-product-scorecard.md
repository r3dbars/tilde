...
Strict manual smoke was green before the latest script-only beta-readiness
guard change. The newest `./script/beta_readiness.sh --check-only` rerun proves
the controls/privacy path no longer self-waits, but correctly treats app/source
proof as stale until the beta-safe rows are refreshed again.
...
| Privacy | 75/100 | Latest `./script/beta_readiness.sh --check-only`: controls/diagnostics, redacted report export, current build privacy export proof, issue template, clipboard fallback disabled, prompt app proof gate, and prompt/chat safety counters passed after the privacy-export proof path stopped waiting on its own beta-readiness parent process. Runtime no-egress proof is now correctly stale because the latest runtime launch is newer than the saved proof and the proof executable SHA-256 no longer matches the expected app binary. `./script/check_runtime_network_egress.py --phase autocomplete --duration 12 --interval 1 --activity-note "current runtime after manual smoke proof refresh" --proof-out docs/product/runtime-network-egress-latest.md --json-out docs/product/runtime-network-egress-latest.json`: passed on 2026-05-28T15:47:49Z with 13 samples, 0 non-loopback remote endpoints, and wrote redacted runtime proof to `docs/product/runtime-network-egress-latest.md` and `docs/product/runtime-network-egress-latest.json` for executable SHA-256 `1bd6224a9752508eebf13a1ccfd12955c1a63d3376d98d472683df8bc7a63782`. `./script/check_sensitive_field_proof_self_test.sh`: passed after the gate expanded required suppressed categories and required browser-hosted suppression rows for Google Docs, Notion, ChatGPT, Slack, Discord, browser login/payment/password-manager/private-search/address-bar/developer-tool, and unproven browser pages. `swift test --jobs 1 --filter 'SensitiveTextFieldPolicyTests|BrowserHostedSurfacePolicyTests'`: 30 tests passed, including the expanded Chrome/Safari/Brave/Arc/Firefox/Chromium browser-hosted fail-closed policy. | Privacy export proof is healthy, but beta privacy is not current until no-egress proof is regenerated after the latest launch/build and onboarding proof is complete. | Rerun runtime no-egress proof, then rerun `./script/beta_readiness.sh --check-only` after onboarding proof and packaged-app Accessibility proof are complete. |
...
| Diagnostics | 92/100 | Latest `./script/beta_readiness.sh --check-only` reached and passed controls/diagnostics after the privacy-export self-wait fix. `./script/check_controls_diagnostics_readiness.sh`: Diagnostics state tests, RawTraceReportExport tests, diagnostics log self-test, redacted report export, and current-build privacy export proof passed. `./script/manual_smoke_status.sh --strict`: passed on 2026-05-28 at commit `e2da385b0ece`, so the beta-safe manual proof rows have recent diagnostics and trace slices, but they must be refreshed after this script/source change. Runtime production gate and redacted report export are OK. | Diagnostics are healthy for local beta work, but no-egress proof, onboarding walkthrough, and packaged-app Accessibility recovery proof are not complete. | Rerun `./script/check_diagnostics_log.sh`, `./script/check_current_build_privacy_export.sh`, runtime no-egress proof, and `./script/manual_smoke_status.sh --strict` after the next app/source change. |
...
| Beta readiness | 75/100 | `AUTOCOMPLETE_LAB_SWIFT_BUILD_JOBS=1 ./script/package_release.sh archive`: created a current Developer ID signed `dist/SteadyType.dmg` and `dist/SteadyType.zip` after the default parallel release build died during compile. `./script/package_release.sh --notarize`: Apple notarization accepted submission `bf39ea78-ed19-4a74-95ba-714ed6c474b6`, stapled the DMG, verified Gatekeeper, and refreshed the secondary ZIP. Latest `./script/beta_readiness.sh --check-only` proves the controls/privacy section no longer deadlocks: model asset, runtime production gate, controls/diagnostics, redacted export, issue template validation, clipboard fallback disabled, production mock fallback disabled, prompt app manifest proof, visual placement proof, release package prerequisites, Developer ID DMG/archive signature, and notarized install proof passed. The same run found 6 real blockers: stale runtime no-egress proof after the latest launch/build, no onboarding walkthrough proof, onboarding permission QA with 48 unchecked items and 3 proof rows waiting for a clean-user run, manual app proof stale after the latest script/source change, no eligible packaged latency launch until Accessibility is granted for the notarized app, and a stale private beta packet commit. `./script/packaged_latency_proof_self_test.sh`: passed and keeps the TextEdit packaged-latency rerun path one command. | Onboarding proof, no-egress refresh, app-proof refresh, private packet regeneration, and packaged Accessibility latency still need current runs before testers should get the build. | Refresh no-egress/manual proof and the private beta packet, grant Accessibility to the notarized app for packaged latency, then record onboarding proof / complete the onboarding checklist and rerun `./script/beta_readiness.sh --check-only`. |
...
| --- | --- | --- |
| Proof freshness | 10 beta-safe target app rows from strict manual smoke. | Recent strict manual smoke passed for all 10 beta-safe rows; latest script/source change makes the gate correctly ask for a refresh before beta release. |
