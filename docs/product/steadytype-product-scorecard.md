...
no visible suggestion after one disposable context. `20260528T035621Z-ghostty`
reached the prompt-row suggestion, delivered Tab, let the native-prefix probe
drain for 8 seconds, proved `prefix-verified=false` at line `828863`, kept the
baseline unchanged at line `828864`, tried `ghosttySendKey` before the slower
native-text fallbacks at lines `828865` / `828866`, and still failed closed at
line `828880` after exceeding the 28s insertion budget. Treat the probe as
unproven until a detached run reaches `ghosttyNativePrefixFinalKeyText` and
verifies one-word no-submit insertion.
