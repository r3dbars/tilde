...
Latest beta gate evidence checked on 2026-05-12 with the strict default-runtime latency selector, `./script/manual_smoke_status.sh --strict`, `./script/private_beta_packet.sh --check`, and `./script/package_release.sh --check --require-developer-id --require-notary-profile`.
...
| Privacy | 90/100 | `./script/beta_readiness.sh --check-only`: redacted report export OK, issue template OK, clipboard fallback disabled OK, prompt app proof gate OK, and prompt/chat safety counters stayed at 0. `./script/check_sensitive_field_proof_self_test.sh` passed after the gate learned to fail unsafe presented `fieldKind` rows even without a sensitive category tag. Runtime classification now passes AX identifier, description, and help through `Sources/AutocompleteLabApp/App/AppDelegate.swift`. | Privacy proof is strong for this loop, but a final beta claim still needs current manual app proof and signed/notarized distribution proof. | Rerun `./script/check_current_build_privacy_export.sh`, `./script/check_runtime_network_egress.py`, and `./script/beta_readiness.sh --check-only` after the remaining beta gates close. |
...
| Beta readiness | 60/100 | `./script/beta_readiness.sh --check-only` now finds 5 blocker(s): latency beta gate blocked, controls/diagnostics readiness blocked during that run, manual app proof blocked by 30 stale or pending rows, package prerequisites blocked by missing `NOTARYTOOL_PROFILE`, and notarized install proof blocked. `./script/package_release.sh --check --require-developer-id --require-notary-profile` found a Developer ID signing identity but failed because `NOTARYTOOL_PROFILE` is missing. `./script/private_beta_packet.sh --check` verified the current packet. | The app has strong local gates, but private beta must stay blocked until current latency proof, current app proof, Apple notarization, stapling, fresh-install Gatekeeper proof, and a clean controls readiness run all pass. | Refresh current manual proof, add notarized install proof, rerun `./script/private_beta_packet.sh --check`, and finish with `./script/beta_readiness.sh --check-only`. |
| Test/proof coverage | 74/100 | `./script/check_test_coverage_manifest.sh`: verified. `./script/check_visual_placement_evidence.sh --require-all`: passed. `./script/check_runtime_network_egress_self_test.sh`: passed. `./script/select_latency_window_self_test.sh`: passed with empty-launch selection coverage and partial-latest fail-red coverage. `./script/latency_benchmark_report_self_test.sh`: passed. `./script/check_sensitive_field_proof_self_test.sh`: passed. `./script/check_proof_manifest.sh --require-all`: failed with 7 issues. `./script/manual_smoke_status.sh --strict`: failed with 30 stale or pending app rows. | The coverage manifest, screenshot audit, latency selector tests, controls diagnostics script, and sensitive-field proof script are stronger, but current proof coverage is still incomplete. | Keep `./script/check_steadytype_scorecard.py`, `./script/check_proof_manifest.sh --require-all`, `./script/manual_smoke_status.sh --strict`, and `./script/beta_readiness.sh --check-only` in the loop until stale proof is refreshed. |
