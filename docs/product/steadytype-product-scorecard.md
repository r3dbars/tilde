...
`front window` lookup. The latest `20260528T083459Z-ghostty` run proved the
pre-launch proof-only reset can remove a Ghostty proof/probe-only pid, then
still failed at `new-window-front-window-start`. Ghostty remains unsupported
until a detached run reaches verified one-word no-submit insertion and exits
`0`.
