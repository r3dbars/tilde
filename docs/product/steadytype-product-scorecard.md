...
Strict manual smoke is green again after the latest proof refresh. The newest
`./script/beta_readiness.sh --check-only` plus individual reruns prove the
manual app proof, runtime no-egress proof, and private beta packet blockers are
gone.
