...
Latest beta gate evidence checked on 2026-05-12 with the sampled default-runtime latency selector, `./script/manual_smoke_status.sh --strict`, `./script/private_beta_packet.sh --check`, and `./script/package_release.sh --check --require-developer-id --require-notary-profile`.
...
| Latency | 88/100 | `./script/beta_readiness.sh --check-only`: latency beta gate passed on 2026-05-12 with the sampled default-runtime selector, `AUTOCOMPLETE_LAB_LOG_START_LINE=20382`, `AUTOCOMPLETE_LAB_TRACE_START_LINE=5485`, first-visible n=8, p95 163 ms, p99 163 ms, first-token p95 126 ms, total-generation p95 195 ms, lateShown=0, and `skippedUnsampledDefaultLaunches=1`. `./script/select_latency_window_self_test.sh` passed after proving under-sampled default launches cannot mask the latest sampled window. | The current slice is fast and resists empty launch noise, but it is still TextEdit-heavy and does not replace broader current-head real-app proof. | Keep `./script/select_latency_window_self_test.sh` and `./script/latency_benchmark_report_self_test.sh` green, then run `./script/beta_readiness.sh --check-only` after each fresh launch so sampled latency cannot drift. |
