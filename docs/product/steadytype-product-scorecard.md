...
| Beta readiness | 80/100 | `./script/private_beta_packet.sh create` previously regenerated the ignored local private-beta packet for the notarized DMG, and `./script/private_beta_packet.sh --check` had passed with stapler, Gatekeeper, app-bundle, model-asset, and checksum validation. The current `./script/beta_readiness.sh --check-only` run now uses the bounded prompt manifest gate instead of the whole historical trace: model asset, runtime production gate, runtime no-egress proof, controls/diagnostics, redacted export, issue template validation, clipboard fallback disabled, production mock fallback disabled, prompt app manifest proof, visual placement proof, release package prerequisites, Developer ID DMG/archive signature, and notarized install proof passed. The same run found 5 real blockers: no onboarding walkthrough proof, onboarding permission QA with 48 unchecked items and 3 proof rows waiting for a clean-user run, out-of-date manual app proof for the current commit, no eligible TextEdit latency window, and an out-of-date private beta packet commit. `./script/packaged_latency_proof_self_test.sh`: passed and now keeps the TextEdit packaged-latency rerun path one command. | Manual app proof, latency, packaging, and the private packet all need current proof after app-source changes, and beta readiness still needs onboarding proof plus the onboarding checklist before testers should get the build. | Refresh strict manual app proof, TextEdit latency, and private beta packet evidence, then record onboarding proof / complete the onboarding checklist and rerun `./script/beta_readiness.sh --check-only`. |
