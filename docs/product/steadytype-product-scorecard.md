...
key so the next insertion-reaching run can distinguish a prefix no-op from a
final-key miss. A follow-up detached run, `20260528T034406Z-ghostty`, failed
earlier with no visible suggestion after one disposable context. Treat the probe
as unproven until a detached run reaches `ghosttyNativePrefixFinalKeyText` and
