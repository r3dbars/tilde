...
| Obsidian long scrolled note | 40 | The lane now creates a long disposable note and reaches the scrolled editor, but no passing screenshot-backed row exists. | Current run suppresses as `stale-text` / `middleOfLine` after CodeMirror line drift in the scrolled document. | Fix scrolled CodeMirror text-context repair so the caret snapshot remains end-of-line, then rerun `obsidian-long-note --manual-gate`. |
