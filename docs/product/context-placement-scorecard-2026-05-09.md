...
Placement-only scorecard for Tilde. This does not grade model
...
| Obsidian default editor | 100 | Current strict visual proof at 2026-05-09T12:53:16Z with two verified accepts in a disposable Obsidian proof note (`manual-smoke-runs.md`, diagnostics lines 261665-261753, trace lines 64880-64905). | None for this default editor lane. | Keep this row green while broader Obsidian variants continue. |
| Obsidian theme variant | 100 | Current strict visual proof at 2026-05-09T12:53:37Z with two verified accepts in `ObsidianProofVault` using the Tilde Proof theme (`manual-smoke-runs.md`, diagnostics lines 261800-261892, trace lines 64914-64939). | None for this theme lane. | Keep this row green while broader Obsidian variants continue. |
| Obsidian split/side pane | 100 | Current strict visual proof at 2026-05-09T12:53:59Z with two verified accepts in a visible split-pane Obsidian window (`manual-smoke-runs.md`, diagnostics lines 261940-262030, trace lines 64948-64973). | None for this pane lane. | Keep this row green while broader Obsidian variants continue. |
| Obsidian long scrolled note | 55 | The lane seeds a real long disposable Markdown note, reaches the scrolled editor, shows a screenshot-backed scrolled suggestion, and verifies the first Tab insertion. The harness now prepares the appended second suffix while Obsidian is out of focus to avoid setup typing over a visible suggestion. | No passing bounded row yet. Current 2026-05-09 retry still fails before the second proof row: after reopening the appended file, CodeMirror AX reports `beforeChars=1011` and `afterChars=21`, so the app correctly blocks with `reason=middleOfLine` instead of guessing. | Fix or replace the long-note caret placement path so the real CodeMirror caret and AX context agree at the file end, then rerun `obsidian-long-note --manual-gate` to create a passing row. |
