...
| Placement reliability | All beta-safe strict target rows are fresh again on `15a6f895d091`: TextEdit, Notes title/body/checklist, Obsidian default/theme/pane/long-note, and Chrome textarea/contenteditable all pass `./script/manual_smoke_status.sh --strict` with strict visual trace evidence; Ghostty detached proof now refuses untrusted top/header synthetic carets, preserves refreshed terminal-screen prompt anchors across proof input repair, bypasses terminal-proof-only final latency suppression, preserves valid in-flight proof requests through focused-text polling throttle, reaches prompt-row suggestion detection through a title-scoped direct prompt anchor when screen-marker text is stale, models the visible Ghostty prompt marker before caret estimation, and survives volatile Ghostty window-title focus by trusting a title-focused root PID before host-app refocus; `20260528T015512Z-ghostty` reached Tab acceptance proof and the insertion ladder, then failed closed because native Ghostty insertions did not mutate the prompt and shell-launched System Events bulk input mutated it unsafely; the fast ladder now tries bundled/CGEvent helpers before that risky shell bulk rung, but later detached runs exposed stale Ghostty cleanup/launch reliability gaps before insertion could be reproved, so Ghostty remains unsupported | Correct or honest fallback in normal writing apps | Run `daily_driver_dogfood_session.sh` in a real writing app, then repair Ghostty stale cleanup plus a verified insertion transport and rerun `./script/claude_code_ghostty_detached_proof.sh start` / `wait` until it proves verified one-word no-submit insertion |
