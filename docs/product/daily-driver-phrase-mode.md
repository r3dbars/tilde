...
suggestion evidence, `20260527T161343Z-ghostty`, repairs the proof input to
`beforeChars=42`, reuses the cached terminal-screen prompt anchor
(`claude-code-terminal-host-proof-prompt-anchor-cache-used`,
`promptLineInputChars=12`, `lineIndex=40`, `lineCount=41`), emits
`synthetic-caret ... source=terminal-screen-prompt` with
`anchorRect=x=130,y=867,w=0,h=22`, and presents a real prompt-row suggestion at
diagnostics line `717840` with `traceID=AFF2D945`, `latencyMilliseconds=939`,
and 8 visible words. It still failed red because the suggestion was no longer
visible before Tab. The latest insertion-transport red proof remains
`20260527T141658Z-ghostty`: it reached hot Tab and the verified insertion ladder,
then failed closed after action text, native input text, send-key, System Events,
bundled helper, Unicode key events, and pasteboard paths all left the disposable
prompt unchanged. The next useful proof slice is to keep the prompt-row
suggestion accept-ready through Tab, then repair or replace the Ghostty insertion
transport.
