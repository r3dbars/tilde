...
`828886` passed with model-backed visible word-completion suggestions for 5
samples: first-visible `n=5 avg=245ms p95=254ms`, first-token `n=5 avg=220ms
p95=229ms`, total generation `n=10 avg=243ms p95=287ms`, event-tap raw
`n=5 avg=40us p99=88us`, AX read summaries `windows=9 samples=540
p99Max=20ms max=23ms`, and zero late shown suggestions, event-tap failures, AX
...
suggestion after one disposable context. `20260528T035621Z-ghostty` reached the
prompt-row suggestion, drained the native prefix probe for 8 seconds, proved
`prefix-verified=false` at line `828863`, kept the baseline unchanged at line
`828864`, tried `ghosttySendKey` before the slower native text fallbacks at
lines `828865` / `828866`, and failed closed on the insertion budget at line
`828880`. This keeps the next red bar honest: rerun until the probe reaches
insertion, then either graduate it with verified no-submit proof or remove it.
