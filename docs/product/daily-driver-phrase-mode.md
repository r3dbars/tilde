...
proof process during exclusive cleanup. `20260527T185409Z-ghostty` found the
fresh prompt-row suggestion at diagnostics line `732838`, captured Tab, and ran
the Ghostty insertion ladder. That is the right next red bar: the now-unblocked
System Events keystroke rung and every other Ghostty insertion transport left
the prompt unchanged, so SteadyType failed closed with `reason=insert-failed`.
The next useful proof slice is a verified one-word no-submit Ghostty insertion
