...
May 26, 2026. The prior live run exposed the wrong trust shape: SteadyType
...
| Placement reliability | All beta-safe strict target rows are fresh again on `15a6f895d091`: TextEdit, Notes title/body/checklist, Obsidian default/theme/pane/long-note, and Chrome textarea/contenteditable all pass `./script/manual_smoke_status.sh --strict` with strict visual trace evidence; Ghostty now has a detached proof runner but the launchd run currently fails before prompt placement because Ghostty exposes no AX text nodes to the helper | Correct or honest fallback in normal writing apps | Run `daily_driver_dogfood_session.sh` in a real writing app, then repair the detached Ghostty AX prompt-readiness path before rerunning `./script/claude_code_ghostty_detached_proof.sh start` / `wait` |
