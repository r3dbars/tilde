...
satisfying the primary scanner, and lets explicit Claude Code terminal-host
proof candidates bypass final-result latency suppression. `20260527T162920Z`
proved the app can now present an 8-word prompt-row suggestion with
`displayScoreLatencySuppressionBypassed=claude-code-terminal-host-proof`.
`20260527T163354Z-ghostty` moved the detached harness to diagnostics line
`722374` and found the prompt-row suggestion, but Ghostty focus churn hid it
before Tab. `20260527T163805Z-ghostty` is the latest one-attempt run after
focus-loss refresh handling; it stayed red because no visible suggestion arrived
in that disposable context. The next useful proof slice is still to keep the
prompt-row suggestion accept-ready through Tab, then repair or replace the
Ghostty insertion transport.
