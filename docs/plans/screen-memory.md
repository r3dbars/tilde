...
PR 3b `claude/redaction-model` — SHIPPED, with two changes from the plan as
originally written, both forced by evidence gathered while building it:
- Core ML conversion of `nvidia/gliner-PII` FAILS (DeBERTa-v3 disentangled
  attention has no Core ML lowering). `script/convert_gliner.py` exports to
  ONNX (opset 17) via gliner 0.2.28's own `export_to_onnx` instead —
  operator-run, same as model packaging, producing a sha256-pinned manifest.
  It also produces an int8-quantized and an fp16 copy for comparison.
- Measured (not assumed) which precision to ship: fp32 clears both eval
  bars at 100%; int8 collapses the unstructured bar to 0% at the shipped
  0.5 threshold (confirms GLiNER issue #218); fp16 fails to load on ONNX
  Runtime's CPU execution provider at all. SHIPPED: fp32. Bundle is
  therefore ~1.78GB, not the ~300–400MB estimated here — flagged for the
  owner, with post-training calibrated (static/QDQ) quantization noted as
  follow-up to recover int8-scale size without the recall collapse.
- Runtime is a small stdin/stdout JSON-lines helper CHILD PROCESS (no
  listening socket, narrower than `LlamaServerProcessHost`'s HTTP+TCP), not
  an in-process linked runtime: GLiNER's pre/post-processing (DeBERTa-v3
  SentencePiece word-splitting, span-grid construction, biaffine decode) is
  ~2,500 lines that only the upstream `gliner` package has been validated
  against; a from-scratch Swift/C++ port could not be cross-checked within
  this PR, and a silently-wrong decode is a PII leak, not a crash. The
  helper is not yet a bundled/pinned `package_app.sh` input — that's
  tracked follow-up, same as this plan already tracks the helper sha
  re-pin per release for llama-server.
- App-side `RedactionService`: rules first, GLiNER spans second (confidence
  ≥ threshold, default 0.5), applied to text BEFORE any persistence.
  Fail-closed: model unavailable for ANY reason (missing assets, launch
  failure, timeout, error reply) → capture dropped, `redaction-helper-*`
  diagnostics (count only, no text).
- Default-excluded apps regardless of user settings: password managers
  (1Password, Bitwarden, Dashlane, LastPass, KeePass*, NordPass, Enpass),
  Keychain Access. List lives in Core (`DefaultExcludedApps`), tested, and
  is unioned into both `CaptureTriggerPolicy` and
  `PersonalHistoryCapturePolicy` so it cannot be bypassed by either
  feature's own exclusion list.
- Redaction eval harness `script/redaction_eval.py` + committed synthetic
  corpus (`script/testdata/redaction_eval_corpus.jsonl`, generated by
  `script/generate_redaction_corpus.py`): blocking bar ≥99% recall
  structured (rules) — met at 100%; ≥90% recall unstructured (model layer)
  — met at 100% with fp32. Not yet wired into `proof.sh` (it needs the
  real model + helper interpreter, which are not CI-available inputs);
  `RedactionCorpusSanityTests` in `swift test` covers the dependency-free
  rules-layer half of the same corpus.
