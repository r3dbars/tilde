...
For beta feedback, attach only that redacted Privacy Bundle when diagnostics are
requested. The structured feedback form does not need raw traces or screenshots
by default. The menu bar `Submit Feedback...` item opens that form, but it does
not attach diagnostics automatically.
